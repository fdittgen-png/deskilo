...
`all`. Scope of this guide: why a pair, creating it, the deploy permissions and the access rule, configuring and testing on the dev, the deployment form, what never travels, instance-level pairing.
...
`env.<module>.<screen>.<object>` on its heading:
```md
### <Heading exactly as the screen calls it> {#env.<module>.<screen>.<object>}
```
...
- An image slot per screen and per explained object:
  ```md
  <p><img src="images/env-<module>-<screen>--<object>.jpg" width="240"></p>
  ```
