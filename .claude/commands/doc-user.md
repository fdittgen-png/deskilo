...
`all`. Scope of this guide: reserve and the plan, calendar, messages, members and profiles, money, events and validations, kiosk and badges, settings.
...
`user.<module>.<screen>.<object>` on its heading:
```md
### <Heading exactly as the screen calls it> {#user.<module>.<screen>.<object>}
```
...
- An image slot per screen and per explained object:
  ```md
  <p><img src="images/user-<module>-<screen>--<object>.jpg" width="240"></p>
  ```
