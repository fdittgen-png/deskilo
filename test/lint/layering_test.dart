...
const Map<String, int> _repositoryInWidgets = {
  'money': 16, // 2026-09-26 #1449 A3 17→16: the expense sheet asks application/submit_expense.dart whether an expense may be filed and files it through that command; 2026-09-20 #1449 18→17: the consumption sheet asks application/record_consumption.dart whether the shelf holds that many and which month the charge lands on — a period that was not a month used to close the sheet and write nothing, silently // 2026-09-20 #1449 20→18: the VAT screen asks application/declare_vat.dart which documents a period declares on its basis, and whether the platform GRANTED the filing; the expense-schedule sheet asks application/schedule_expense.dart what answering at a different amount means, and is refused when the explanation is missing // 2026-09-19 #1449 21→20: the settlement sheet asks application/settle_invoices.dart which invoices may be regrouped and what the new document is called // 2026-09-19 #1449 23→21: Repartitions is a class behind a provider, so BOTH the wizard and the expense sheet stopped resolving the repository — the free-function form had left each caller reading one // 2026-09-19 #1449 24→23: the legal-identity screen asks application/save_legal_identity.dart which aggregate is written first, and resolves no repository at all // 2026-09-18 #1449 25→24: the register-payment sheet asks application/record_payment.dart which month a payment belongs to
  'workspace': 18, // 2026-09-20 #1449 19→18: the onboarding wizard asks application/start_workspace.dart what a creation needs before it is one, and sends the request id that makes a retry the SAME creation rather than a second dev/prod pair // 2026-09-20 #1449 20→19: the deployment screen asks application/deploy_configuration.dart what a tick carries and deploys the PREVIEWED set, never the ticks as they stand when the sheet closes // 2026-09-19 #1449 21→20: the new-conversation sheet asks application/start_conversation.dart what a second person means and whether a taken name is a correction; it resolves no repository // 2026-09-19 #1528 20→21: the screen that defines a workspace's own roles resolves the roles repository, the same shape the questions editor has and the same one every settings screen has; // 2026-09-19 #1288 S4 19→20: the question editor screen resolves the fields repository to define a question — the same shape every settings screen has, and the decision it makes is the server's; // 2026-09-17 #1280 S3 20→19: the save sheet publishes through application/publish_template.dart
  'auth': 4,
