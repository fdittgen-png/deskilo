...
void main() {
  ...
  testWidgets(
    ...
    await openSheet(tester, 'Ana');
    await tester.tap(find.text('May not reserve a whole desk, office or level'));
    await tester.pumpAndSettle();
    expect(workspace.levelPermissions['member-2'], isTrue);
    expect(workspace.otherMembers.single.canReserveLevel, isTrue);

    await openSheet(tester, 'Ana');
    await tester.tap(find.text('May reserve a whole desk, office or level'));
    await tester.pumpAndSettle();
  ...
  testWidgets(
    ...
    expect(find.text('May not reserve a whole desk, office or level'), findsOneWidget);
  });
