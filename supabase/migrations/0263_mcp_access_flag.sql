...
-- risk: additive
--
-- 0263 (#1607) -- `mcpAccess` joins the server's feature registry.
--
-- The flag says that the MCP interface is AVAILABLE to a workspace: an
-- AI assistant may be connected to DesKilo through it. It says nothing
-- else. Switching it on enrols nobody, approves no request, grants no
-- role and registers no OAuth client; the access contract of the whole
-- MCP batch is
--
--   instance enabled AND workspace feature effective AND registered
--   non-anonymous active user AND active membership AND current
--   owner-configured grant approved by the instance administrator AND
--   approved OAuth client / user consent AND operation within the grant
--   ceiling AND the application permission, feature and business
--   validation the operation already answers to,
--
-- and this flag is the second conjunct alone. Every later ticket asks
-- `public.feature_effective(workspace_id, 'mcpAccess')` and never reads
-- the jsonb directly, so the parent chain, the default and the stored
-- override resolve in one place.
--
-- Platform tier, default OFF, requiring nothing: availability depends on
-- no other functionality, and what a connected assistant may DO is
-- decided per call by the feature and permission it touches. A
-- workspace is never reachable by a machine it did not ask to be
-- reachable by.
--
-- No endpoint, no RPC, no table, no grant record and no gate is created
-- here -- deliberately. The flag has to exist before anything can be
-- gated on it, and a gate that arrived in the same migration as the flag
-- would have nothing to protect yet. Nothing that could hold a grant,
-- an approval, a consent or an administrator identity travels with a
-- template either: the definition is ordinary configuration, and
-- enabling it on a clone opens nothing.
--
...
select public.set_deskilo_schema_version(263);
