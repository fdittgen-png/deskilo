...
-- risk: additive
--
-- 0267 (#1654) -- `memberGettingStarted` joins the server's feature registry.
--
-- The flag says that a newly joined (or newly created) workspace shows
-- its member ONE compact, dismissible card on the Reserve hub: which
-- workspace they are in, and one suggested next action -- choose a time
-- to book, read their own membership, or open the help -- only where the
-- features, the permissions and the data the hub already loaded support
-- it. The card writes nothing but its own dismissal, to the device's
-- help-hint store; it never submits a booking, a payment or an approval.
--
-- Core tier, default ON, requiring nothing: optional help every space
-- meets on day one and can hide. OFF hides the card and changes nothing
-- else -- no access, no booking data, no permission follows the flag.
--
-- No endpoint, no RPC, no table and no policy travel with this: the card
-- is a READ of what the hub already shows, and the client gate reads the
-- flag through the same registry every other gate does. The server's copy
-- exists so that `feature_effective` and the templates name the flag like
-- any other.
--
...
select public.set_deskilo_schema_version(267);
