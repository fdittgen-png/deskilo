...
-- review.
--
-- import_floor_plan transactionally REPLACES a workspace's whole floor
-- plan from a jsonb payload mirroring the XML interchange format (#164):
-- an array of levels, each nesting offices[desks[seats[amenities]]].
-- Ids are regenerated; names, sort order, geometry, orientation, chair
-- and maintenance blocks are preserved. Seat amenities land in
-- seats.amenities (text[]) exactly as the editor writes them today —
-- accessory-catalog linking (0022) is out of scope here; the catalog
-- rows themselves survive the replace untouched.
--
-- Safety (v1): a workspace with ANY reservation row refuses the import.
-- reservations.seat_id / office_id are ON DELETE RESTRICT (0005) because
-- billing counts reservations — even cancelled/completed rows must keep
-- their seat. Rather than surface a raw FK error, we raise the typed
-- 'workspace has reservations' the client maps to a clear message.

create or replace function public.import_floor_plan(
  p_workspace_id uuid,
  p_plan jsonb
...
begin
  ...
  -- v1 imports only into reservation-free workspaces: reservations
  -- reference seats/offices (0005, on delete RESTRICT) and billing counts
  -- them, so history must never be destroyed. ANY status counts —
  -- cancelled and completed rows still reference their seat.
  if exists (
  ...
  -- The accessories catalog (accessories, 0022) references only the
  -- workspace and is left intact.
...
grant execute on function public.import_floor_plan(uuid, jsonb) to authenticated;
revoke execute on function public.import_floor_plan(uuid, jsonb) from public, anon;
