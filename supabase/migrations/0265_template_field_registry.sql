...
-- risk: additive
--
-- 0265 (#1655) — the field registry, as the database reads it.
--
-- The client's `templateFieldRegistry()` says, per field a template can
-- carry, whether it travels as written, what its absence means, whether it
-- is a reference or a target-local binding, and why it never travels.
-- `inspect_workspace_template` (0266) reads the same records here, so
-- `public.template_field_registry()` is GENERATED from the Dart
-- (`dart run tool/build_template_field_registry_sql.dart`) and
-- `template_field_registry_sql_test` requires the latest migration defining
-- it to carry exactly that text. A registry change is a new migration
-- restating this function; the inspector's own file never moves for it.
...
as $registry$
  select $json$[
    ...
    {"id":"floor_plan[].offices[].desks[].seats[].orientation","entity":"floor_plan","type":"integer","portability":"literal","absent":"inherit","key":"name","process":"spaceManagement"},
    ...
    {"id":"workspace.desk_opacity","entity":"booking_rules","type":"decimal","portability":"literal","absent":"product_default","min":0,"max":1,"process":"reservationsUsage"},
...
select public.set_deskilo_schema_version(265);
