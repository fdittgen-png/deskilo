...
-- risk: additive
--
-- 0253 (#1247) — `decisionSurface` joins the server's feature registry.
--
...
-- statement verbatim. That is the point of the generator: the server's
-- copy of the flag tree cannot drift from the client's by hand.
--
-- Platform tier, default OFF, requiring nothing. A space with two
-- members and no invoicing does not want a decision surface, and each
-- row's own capability already gates its data — a surface that needed
-- every feature switched on to be useful would be the wrong surface.
...
select public.set_deskilo_schema_version(253);
