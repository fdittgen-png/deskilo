...
-- risk: additive
--
-- 0261 (#1598) -- `memberAccountMenu` joins the server's feature registry.
--
-- The Pezenas field report asks that an ordinary member meet no
-- "Membres" and no "Reglages". The directory already answers to
-- `membersDirectory`; the gear did not -- the shell rendered it for
-- everybody and pushed /settings unconditionally.
--
-- While this flag is on, a member whose EFFECTIVE permission set is
-- empty meets "My account" where the gear was: the same icon slot, the
-- same drawer entry, the same /settings route, under the name that says
-- whose page it is. The screen has asked the permission matrix since
-- #1307, so such a member was already shown nothing but their own
-- account, membership and preferences -- account and profile, sign-out,
-- language and regional preferences, workspace switching, privacy and
-- Help all stay exactly where they were. Anyone whose role grants an
-- administration -- including a custom role, which grants additively
-- into the same set -- keeps Reglages and everything it opens.
--
-- Platform tier, default OFF, requiring nothing: the gear it renames is
-- unconditional, so there is no parent whose absence would make the
-- choice meaningless, and a space that never asked keeps what it has.
--
-- NO server gate reads this flag, and that is deliberate. Hiding an
-- entry is not an access boundary: the boundary is `has_permission` and
-- the RLS policies that consult it, which answer the same before and
-- after this migration. A flag that started refusing rows would make a
-- NAMING choice into a permission, and an owner who switched it on
-- would silently withdraw an access nobody meant to withdraw. The
...
select public.set_deskilo_schema_version(261);
