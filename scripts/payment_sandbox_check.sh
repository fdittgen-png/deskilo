...
# takes verbatim. Q-018 / Q-019 add a provider case each.
set -uo pipefail
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    *) echo "usage: $0 --provider <stripe> --backend <project ref>"; exit 2 ;;
  esac
done
...
missing=""
for v in STRIPE_TEST_SECRET_KEY STRIPE_TEST_WEBHOOK_SECRET SANDBOX_URL SANDBOX_ANON_KEY SANDBOX_SERVICE_ROLE_KEY SANDBOX_DB_URL; do
...
sql "insert into public.payment_credentials (workspace_id, provider, config) values ('$WS', 'stripe',
     json_build_object('secret_key', '$STRIPE_TEST_SECRET_KEY', 'webhook_secret', '$STRIPE_TEST_WEBHOOK_SECRET',
                       'return_url', 'https://example.test/return')::jsonb)" >/dev/null \
...
code=$(curl -s -o "$WORK/out" -w '%{http_code}' -X POST "$SANDBOX_URL/functions/v1/create-payment-order" \
  ...
  -d "{\"workspace_id\":\"$WS\",\"provider\":\"stripe\",\"member_id\":\"$MEMBER\",\"amount_cents\":100,\"period\":\"$(date -u +%Y-%m)\",\"currency\":\"EUR\"}")
...
echo "session $SESSION opened in test mode (API version ${API_VERSION:-unknown})"
echo
echo "ACTION REQUIRED — pay the session by hand, in a browser, with a Stripe TEST card:"
echo "  1. open $URL"
echo "  2. card 4242 4242 4242 4242, any future expiry, any CVC, any postcode"
...
for _ in $(seq 1 $(( ${PAYMENT_SANDBOX_WAIT:-600} / 5 ))); do
  status=$(sql "select status from public.payment_intents where provider = 'stripe' and order_id = '$SESSION'")
  [ "$status" = "created" ] || break
  sleep 5
done
case "$status" in
  captured) ;;
  failed) fail "Stripe reported the session as failed or expired; the intent is marked failed and nothing was credited." ;;
...
record pass pass "One Checkout Session opened by the deployed create-payment-order, paid with a test card, settled once by the deployed stripe-webhook: captured, one associated posting, reconciliation clean. No real charge: test mode only."
echo "pass"
}
