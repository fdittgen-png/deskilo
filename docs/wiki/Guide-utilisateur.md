...
- **Modèle de PDF de facture** (propriétaire, icône crayon dans l'en-tête Factures) — rédigez une introduction (au-dessus du bloc destinataire) et un pied de page (conditions de paiement, mentions légales) rendus sur chaque PDF de facture, avec des espaces réservés comme `{{member}}`, `{{number}}`, `{{period}}` ou `{{total}}`. Le XML de facture électronique n'est jamais modifié.
