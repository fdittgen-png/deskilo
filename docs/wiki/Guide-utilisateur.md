...
**Un scan ouvre la feuille de la borne.** Lire un code d'espace — la carte QR imprimée, ou l'étiquette NFC collée sur le siège — propose exactement ce que propose la borne quand on touche cet espace : mêmes actions, même période déduite des réglages de l'espace. Seule différence : vous êtes déjà connecté, donc pas d'étape badge (§4b).
...
| Avant l'ouverture / après les heures (début 6:00, 17–21) | ❌ | ❌ | ✅ par défaut — les grilles sont libres ; la règle **Réservations à la minute dans les heures d'ouverture** (§8) transforme cela en ❌. Et sur **toutes** les granularités, la règle **En dehors des heures d'ouverture** (§8) décide de ce que vaut une fenêtre hors horaires acceptée — interdite, gratuite ou facturée (ci-dessous) |
...
**En dehors des heures d'ouverture.** Une fenêtre située **entièrement** hors de la journée de travail — un petit matin 6:00–8:00, une soirée 17:00–21:00, la prolongation spontanée qui court jusqu'à minuit local — relève d'une règle unique de l'espace, à trois positions (§8). Une fenêtre qui touche les heures de travail, ne serait-ce que d'une minute, est une réservation ordinaire : elle n'est pas concernée.

| Position | Une réservation (ou un pointage spontané) entièrement hors horaires |
|---|---|
| **Interdit** | ❌ refusée sur toutes les granularités — y compris la prolongation du soir que les granularités par journées permettent pourtant toujours |
...
Cette exemption est tout l'intérêt du défaut : elle empêche de « ne réserver qu'en dehors des heures pour ne pas payer » sans faire payer deux fois un membre qui a déjà consommé sa journée. À ne pas confondre avec **Réservations à la minute dans les heures d'ouverture** (§8) : celle-ci confine les réservations *sur grille de minutes* **à l'intérieur** de la journée ; celle-là gouverne les fenêtres situées **entièrement en dehors**, sur toutes les granularités.

**Les arrivées spontanées s'alignent sur le créneau.** Une arrivée spontanée (toucher une place libre, scanner son QR/NFC, ou la borne) réserve de *maintenant* jusqu'à un bord canonique — la limite de demi-journée, la fin de journée, ou un bord de grille. En granularité par journées, la réservation couvre le **créneau entier auquel la fin appartient** : arriver à 10:00 et choisir *jusqu'à 12:00* réserve toute la matinée 8:00–12:00 ; ce n'est que si le début est réellement pris par quelqu'un d'autre que la réservation s'ancre à votre arrivée. À la fin de la journée de travail ou après, une arrivée spontanée peut courir jusqu'à **minuit local** (prolongation du soir — sur toutes les granularités, sauf si **En dehors des heures d'ouverture** est sur *Interdit*, la seule règle qui la refuse). Et un check-in spontané doit commencer **aujourd'hui** : créer une réservation « pointée » pour demain est refusé.

**Un scan se comporte comme la borne.** Scanner le code d'une place, d'une table, d'un bureau ou d'un niveau — QR imprimé ou étiquette NFC — ouvre la feuille même que la borne ouvre quand on touche cet espace : mêmes actions, mêmes périodes déduites, sans l'étape badge, puisque vous êtes déjà connecté. Ensuite, c'est l'espace qui décide :
...
- **Les miennes / Tout le monde** — vos propres réservations, ou celles de toute la communauté. Les points sous un jour disent tout d'un coup d'œil : **rouge** = vous avez une réservation, **bleu** = d'autres membres en ont, **les deux points** = les deux. Aujourd'hui est cerclé.
...
- **Règles de réservation** — cinq entrées qui assouplissent ou resserrent les règles du §4b (la section suit la fonctionnalité *Règles de réservation*) ; les trois interrupteurs sont tous **coupés par défaut** :
  ...
  - **En dehors des heures d'ouverture** — trois positions pour une fenêtre située entièrement hors de la journée de travail (un petit matin, la soirée, la prolongation spontanée jusqu'à minuit). *Interdit* refuse ces réservations et ces pointages spontanés sur toutes les granularités ; *Gratuit* les permet sans jamais les compter ni les facturer ; *Facturé* (le **défaut**) les compte comme un usage ordinaire — sauf un jour où le membre tient déjà une réservation normale dans les horaires : la partie hors horaires passe alors gratuitement.
...
- **Le questionnaire de configuration** — <https://fdittgen-png.github.io/deskilo/setup.html> : une page autonome (Mac, PC ou téléphone ; réponses enregistrées automatiquement dans le navigateur) qui guide un nouveau propriétaire à travers **tous les sujets, en choix prédéfinis** — identité (pays dont la Norvège, devise, fuseau), disponibilité et granularité, plan des locaux, toutes les fonctionnalités (y compris les déclarations de TVA), paliers de facturation et niveaux d'abonnement, forfaits, services et accessoires, instructions de paiement, **identité légale et TVA** (type d'organisation, régime, taux usuels du pays — le 3,8 % hébergement suisse, la Norvège, les provinces canadiennes, avec la note honnête sur la sales tax américaine —, mentions de facturation, règles de relance), la matrice rôle → permission, la règle de validation, et les membres à inviter. **Exporter le XML** : l'app importe réglages, accessoires et plan (*Importer l'espace (XML)*) ; la section `<setup>` du fichier porte le reste pour finir la configuration. La page peut aussi **recharger** un fichier exporté pour continuer l'édition.
