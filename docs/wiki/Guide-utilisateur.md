...
2. Le propriétaire (ou un admin) émet un **badge** par membre : un QR code affiché **une seule fois** — imprimez-le en carte ou gardez-le sur votre téléphone. Un badge se révoque à tout moment.
3. À la borne : touchez une place → **Arrivée**, **Réserver** ou **Départ** → présentez votre badge. Un lecteur de codes-barres USB/Bluetooth le lit instantanément (il tape le code), ou saisissez le code à la main.

Votre identité n'existe que le temps de l'opération : le code du badge est envoyé une fois au serveur, la réservation est faite **à votre nom**, et rien n'est stocké sur la tablette — vous êtes « déconnecté » dès que c'est terminé. Le scan par caméra, le badge NFC (Android — les iPad n'ont pas de puce NFC) et la connexion ponctuelle Google/Facebook sont sur la feuille de route.
