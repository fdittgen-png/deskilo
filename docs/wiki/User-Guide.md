...
**How documents are numbered (#925).** Each journal — invoices, credit notes — has its own **series**, taken in the database the moment a document is issued: gapless (a document that fails takes nothing), exact however many admins issue at once, and on the year of **your** timezone, not UTC. The owner sets the format on one screen, *Settings → Number sequences*: prefix, year or month, digits, when the counter restarts, with a preview of the next number. The format applies to what comes next; an issued document never changes, and the counter can be raised but never lowered.
