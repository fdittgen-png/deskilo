...
- **Mine / Everyone** — your own bookings, or the whole community's. Your days are marked **red**, other members' **blue**, today is ringed; a dot under a day means something is booked there.
...
**Messages:** the bell also collects your member notifications (§6) — received and sent, newest first. The list shows only the **first 64 characters**; **tap a message** (or **swipe right**) to open the **conversation** with that member — the complete exchange as chat bubbles, emojis and reference links live (a reservation link opens that reservation, a space link opens the booking sheet), with the composer right below; a broadcast opens as a single full message instead. **Swipe left** to delete a message (a long-press on a bubble deletes from the thread too) — deleting always **asks for confirmation** first (a received all-admins broadcast can't be deleted — it would vanish for every admin).  Your own messages carry a small check next to the time: **grey = delivered**, **blue = read** by the recipient (a broadcast to all admins stays grey — it has many readers). Unread messages count on the bell and on the app icon until you open this screen.
...
4. At the kiosk, tap a seat (or **This level**) → **Check in**, **Reserve**, or **Check out**. For check-in and reserve a **period step** comes first — you say *when* before any badge comes out, **today only** and true to the workspace granularity: under half-days, **Morning / Afternoon / Day** chips (a running window starts *now*, past ones are disabled, after hours a single *Rest of the day* remains); under timed granularities, **From/To** pickers snapped to the slot grid — checking in means being there, so its start is pinned to *now* and only the end moves. Reserving a window that has already begun also asks **Check in right away?** (on by default — you are, after all, standing at the kiosk): confirming books the reservation *already checked in*, with one single badge presentation. Then present the badge:
