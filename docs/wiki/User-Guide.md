...
**A scan opens the kiosk's own sheet.** Reading a space code — the printed QR, or the NFC tag stuck on the chair — offers exactly what tapping that space on the kiosk offers: the same actions, the same period derived from the workspace settings. The one difference is that you are already signed in, so there is no badge step (§4b).
...
| Before opening / after hours (6:00 start, 17–21) | ❌ | ❌ | ✅ by default — the grids are free-time; the **Minute bookings within working hours** policy (§8) turns this into ❌. And on **every** granularity, the **Outside the opening hours** policy (§8) decides what an accepted outside-only window is worth — refused, free, or charged (below) |
...
**Outside the opening hours.** A window lying **entirely** outside the working day — a 6:00–8:00 early morning, a 17:00–21:00 evening, the overtime walk-up running to local midnight — is governed by one workspace-wide, three-way policy (§8). A window that touches the working hours at all, even by a minute, is an ordinary booking and is untouched by it.

| Mode | A booking (or walk-up check-in) lying wholly outside the hours |
|---|---|
| **Off** | ❌ refused on every granularity — including the evening overtime walk-up that the day-based granularities otherwise always allow |
...
That exemption is the point of the default: it stops "book only outside the hours to avoid paying" without charging twice a member who already used their day. Do not confuse it with **Minute bookings within working hours** (§8): that policy confines *minute-grid* bookings **within** the working day; this one governs windows lying **wholly outside** it, on every granularity.

**Walk-ups snap to the slot.** A walk-up (tap a free seat, scan its QR/NFC, or the kiosk) books *now* until a canonical edge — the half-day boundary, the day end, or a grid edge. Under day-based granularities the booking covers the **whole slot the end belongs to**: arriving at 10:00 and choosing *until 12:00* books the full 8:00–12:00 morning; only when the early part is genuinely taken by someone else does the booking anchor at your arrival instead. At or after the working day's end a walk-up may run to **local midnight** (evening overtime — on every granularity, unless **Outside the opening hours** is *Off*, the one policy that refuses it). And a walk-up check-in must start **today**: creating a "checked-in" booking for tomorrow is refused.

**A scan behaves like the kiosk.** Scanning a seat, table, office or level code — printed QR or NFC tag — opens the very sheet the kiosk opens when that space is tapped: the same actions, the same derived periods, minus the badge step, because you are already signed in. From there the space decides:
...
The same *message the holder* action sits on the **Plan tab** when you tap a seat someone else occupies. At the kiosk the receipt names the holder and points you to the app instead: a wall device never sends messages for you.
...
- **Booking policies** — five entries that relax or tighten the rules of §4b (the section rides the *Booking policies* feature); the three switches are all **off by default**:
  ...
  - **Outside the opening hours** — three choices for a window lying entirely outside the working day (an early morning, the evening, the overtime walk-up to midnight). *Off* refuses such bookings and walk-up check-ins on every granularity; *Free* allows them but never counts or charges them; *Charged* (the **default**) counts them like ordinary usage — except on a day where the member already holds a regular inside-hours booking, when the outside part rides free.
...
Tap a member to open their **management sheet** — every per-member action in one place: **Send the financial agreement** (§11d), **Messages**, **Add a service** (service, quantity, billing month → *submit for confirmation*), **Subscription** (their percentage), **When the days run out** (the over-consumption policy, §9), **Reservation limit** (how many open reservations at once), **Simultaneous reservations** (how many *overlapping* bookings — the personal allowance that overrides the workspace number, §4b), **May reserve a whole table, desk or level**, **Badges** (§10), **Name admin** (validated, §7), **Co-ownership**, **Turn into a kiosk**, and **Pause the membership**. Each row shows the member's **email** under the name.
