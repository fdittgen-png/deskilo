...
| Before opening / after hours (6:00 start, 17–21) | ❌ | ❌ | ✅ by default — the grids are free-time; on **every** granularity the **Outside the opening hours** policy (§8) decides what such a window is worth, or whether it exists at all — refused, walk-up only, free, or charged (below) |
...
| **Spontaneous only** | ✅ the walk-up check-in, evening overtime to midnight included — ❌ reserving that window **ahead**, and ❌ a booking spilling past the day's end |
...
- **Booking policies** — four entries that relax or tighten the rules of §4b (the section rides the *Booking policies* feature); the two switches are both **off by default**:
  ...
  - **Outside the opening hours** — one question, four mutually exclusive answers, the same on every granularity: *what may happen outside the working day?* **Off** — nothing: no booking ahead, no walk-up, and a booking spilling past the day's end (or starting before it opens) is refused too. **Spontaneous only** — the walk-up check-in stays possible, evening overtime to midnight included, while reserving ahead outside the hours is refused; this is where the old **Minute bookings within working hours** switch went, and workspaces that had it on read as this. **Free** — allowed, never counted and never charged (pure presence information). **Charged** (the **default**) — counted like ordinary usage, except on a day where the member already holds a regular inside-hours booking, when the outside part rides free.
