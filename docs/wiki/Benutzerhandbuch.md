...
**Ein Scan öffnet das Blatt des Kiosks.** Einen Raumcode zu lesen — die gedruckte QR-Karte oder den NFC-Tag am Stuhl — bietet genau das an, was der Kiosk beim Antippen dieses Raums anbietet: dieselben Aktionen, denselben aus den Space-Einstellungen abgeleiteten Zeitraum. Der einzige Unterschied: du bist bereits angemeldet, also entfällt der Badge-Schritt (§4b).
...
| Vor Öffnung / nach Feierabend (Start 6:00, 17–21) | ❌ | ❌ | ✅ standardmäßig — die Raster sind frei; die Regel **Minutenbuchungen innerhalb der Arbeitszeiten** (§8) macht daraus ❌. Und auf **jeder** Granularität entscheidet die Regel **Außerhalb der Öffnungszeiten** (§8), was ein zugelassenes Fenster außerhalb der Zeiten wert ist — abgelehnt, gratis oder berechnet (unten) |
...
Und auf jeder Granularität gleichermaßen: die Zukunft ist offen bis zum **Buchungshorizont** (Standard 90 Tage) und dahinter gesperrt; eine Buchung an einem **bereits beendeten Tag** (gestern und früher) wird abgelehnt — *„liegt vollständig in der Vergangenheit"* — außer die Inhaberin hat **Vergangene Buchungen erlauben** eingeschaltet, während das Fenster von heute Vormittag später am selben Tag immer buchbar bleibt; ein **Schließtag** lehnt mit Namen ab; ein belegter Platz lehnt ab, und ein Mitglied hält nur so viele **sich überschneidende** Buchungen, wie sein Kontingent erlaubt (§4).

**Wie viele Plätze gleichzeitig.** Der Space legt eine Zahl **gleichzeitiger Reservierungen** fest (§8); sie ist standardmäßig **1** — genau der bisherige eine Platz zur Zeit. Eine Inhaberin oder ein Admin kann einem einzelnen Mitglied in *Mitglieder & Pläne* ein höheres Kontingent gewähren, und diese persönliche Erlaubnis sticht die Zahl des Space; niemand setzt die eigene. Dasselbe Kontingent regiert die **Check-ins**: wer 2 Plätze halten darf, kann an 2 Plätzen zugleich eingecheckt sein. Ist das Kontingent erreicht, folgt die gewohnte Ablehnung — *du hast in diesem Zeitraum bereits eine Reservierung* bzw. *bereits woanders eingecheckt*.

**Außerhalb der Öffnungszeiten.** Ein Fenster, das **vollständig** außerhalb des Arbeitstags liegt — ein früher Morgen 6:00–8:00, ein Abend 17:00–21:00, die Spontan-Überstunde bis Mitternacht (Ortszeit) —, unterliegt einer einzigen, dreistufigen Regel des Space (§8). Ein Fenster, das die Arbeitszeiten auch nur um eine Minute berührt, ist eine gewöhnliche Buchung und davon unberührt.

| Stufe | Eine Buchung (oder ein Spontan-Check-in) ganz außerhalb der Zeiten |
|---|---|
| **Aus** | ❌ auf jeder Granularität abgelehnt — auch die Abend-Überstunde, die tagesbasierte Granularitäten sonst immer erlauben |
...
Diese Ausnahme ist der Sinn des Standards: sie unterbindet „nur außerhalb der Zeiten buchen, um nicht zu zahlen", ohne ein Mitglied doppelt zu belasten, das seinen Tag schon verbraucht hat. Nicht zu verwechseln mit **Minutenbuchungen innerhalb der Arbeitszeiten** (§8): jene Regel hält *Minutenraster*-Buchungen **innerhalb** des Arbeitstags; diese regiert Fenster, die **ganz außerhalb** liegen — auf jeder Granularität.

**Spontan-Check-ins rasten auf den Slot ein.** Ein Spontan-Check-in (freien Platz antippen, QR/NFC scannen, oder am Kiosk) bucht von *jetzt* bis zu einem kanonischen Rand — der Halbtagsgrenze, dem Tagesende oder einem Rasterrand. Bei tagesbasierter Granularität deckt die Buchung den **ganzen Slot, zu dem das Ende gehört**: um 10:00 ankommen und *bis 12:00* wählen bucht den vollen Vormittag 8:00–12:00; nur wenn der frühe Teil wirklich von jemand anderem belegt ist, ankert die Buchung stattdessen an deiner Ankunft. Am oder nach dem Ende des Arbeitstags darf ein Spontan-Check-in bis **Mitternacht (Ortszeit)** laufen (Abend-Überstunden — auf jeder Granularität, außer **Außerhalb der Öffnungszeiten** steht auf *Aus*, die einzige Regel, die sie ablehnt). Und ein Spontan-Check-in muss **heute** beginnen: eine „eingecheckte" Buchung für morgen wird abgelehnt.

**Ein Scan verhält sich wie der Kiosk.** Den Code eines Platzes, Tischs, Büros oder einer Etage zu scannen — gedruckter QR oder NFC-Tag — öffnet genau das Blatt, das der Kiosk beim Antippen dieses Raums öffnet: dieselben Aktionen, dieselben abgeleiteten Zeiträume, ohne den Badge-Schritt, denn du bist schon angemeldet. Danach entscheidet der Raum:
...
**Validierungsquorum:** für Geld und Rollen definiert die Inhaberin, *wer* zustimmen muss und *wie viele*. **Niemand validiert das eigene Ereignis** — nur eine andere Person (eine von der Inhaberin konfigurierte Ausnahme für Reservierungslöschungen, unten); ohne anderen Validierer wartet der Antrag. Unbeantwortetes verfällt nach 7 Tagen — nichts Teures wird still gewährt, nichts selbst gewährt.
...
- **Buchungsregeln** — fünf Einträge, die die Regeln aus §4b lockern oder straffen (der Abschnitt folgt der Funktion *Buchungsregeln*); die drei Schalter sind alle **standardmäßig aus**:
  ...
  - **Außerhalb der Öffnungszeiten** — drei Stufen für ein Fenster, das ganz außerhalb des Arbeitstags liegt (ein früher Morgen, der Abend, die Spontan-Überstunde bis Mitternacht). *Aus* lehnt solche Buchungen und Spontan-Check-ins auf jeder Granularität ab; *Gratis* erlaubt sie, ohne sie je zu zählen oder zu berechnen; *Berechnet* (der **Standard**) zählt sie wie gewöhnliche Nutzung — außer an einem Tag, an dem das Mitglied schon eine reguläre Buchung innerhalb der Zeiten hält: der Teil außerhalb fährt dann gratis mit.
  - **Gleichzeitige Reservierungen pro Mitglied** — wie viele sich überschneidende Buchungen ein Mitglied halten darf, Check-ins eingeschlossen. **1** standardmäßig: ein Platz zur Zeit. Eine Inhaberin oder ein Admin kann einem einzelnen Mitglied in *Mitglieder & Pläne* ein höheres Kontingent gewähren (nie sich selbst), und diese persönliche Erlaubnis sticht diese Zahl.
