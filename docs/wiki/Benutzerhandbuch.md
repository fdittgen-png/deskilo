...
- **Benachrichtigung senden** (in der Detailkarte und auf den Mitgliederkarten von *Mitglieder & Pläne*): eine kurze Notiz an ein anderes Mitglied — zugestellt als Push und als Benachrichtigung mit deinem Namen und deiner Nachricht. Admins haben ein Megafon **Alle Admins benachrichtigen** in der Kopfzeile, das alle Admins inkl. Inhaber erreicht. Schaltbar über die Funktion *Mitglieder-Benachrichtigungen*.
...
- **Rechnungs-PDF-Vorlage** (Inhaber, Stift-Symbol in der Rechnungs-Kopfzeile) — Einleitung (über dem Empfängerblock) und Fußtext (Zahlungsbedingungen, Pflichtangaben) für jedes Rechnungs-PDF, mit Platzhaltern wie `{{member}}`, `{{number}}`, `{{period}}` oder `{{total}}`. Das E-Rechnungs-XML bleibt unangetastet.
