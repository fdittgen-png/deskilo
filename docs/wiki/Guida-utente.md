...
**Prima i processi** (#1327). La schermata si apre con una scheda per ogni processo aziendale — *Spazio e accesso*, *Fatturazione e pagamenti* e gli altri — invece che con un centinaio di interruttori. Ogni scheda dice il suo stato a parole accanto a un'icona: **Attivo** (ogni funzionalità del processo funziona), **Parziale** (alcune), **Disponibile** (nessuna è ancora attiva) oppure **Da verificare** (una funzionalità è attiva ma attende un prerequisito disattivato). Conta i sottoprocessi attivi e le funzionalità attive, e avvisa quando attivare l'intero processo richiederebbe anche una funzionalità di un altro processo. Tocca una scheda per vederne i sottoprocessi e cosa fa ogni funzionalità; tocca una funzionalità per raggiungere il suo interruttore. Un solo campo di ricerca raggiunge processi, sottoprocessi e funzionalità, e mostra ogni risultato sotto il suo percorso (*Spazio e accesso › Accesso fisico*); i chip **Tutti**, **Attivo**, **Disponibile** e **Da verificare** restringono le schede — un processo parziale conta sia come attivo sia come disponibile. La vista **Interruttori**, sopra l'elenco, conserva ogni interruttore, la loro ricerca e il chip **Modificate**. **Attivare o disattivare un processo (#1329).** Una scheda aperta offre **Attiva** e **Disattiva** per l'intero processo e per ogni sottoprocesso. Entrambi aprono la stessa anteprima prima di scrivere qualsiasi cosa: cosa viene cambiato, cosa è *necessario anche* e arriva da un altro processo, cosa *torna a funzionare* perché il suo prerequisito ritorna, e cosa è *già attivo*. Disattivare qualcosa di cui altre funzionalità hanno ancora bisogno viene prima rifiutato: scegliete se lasciare tutto com'è, disattivare ciò che avete chiesto conservando le impostazioni delle dipendenti (smettono di funzionare finché il prerequisito non torna), oppure disattivare anche le dipendenti. La scrittura avviene contro esattamente ciò che l'anteprima ha mostrato; se qualcuno ha modificato le funzionalità nel frattempo, non viene scritto nulla, l'anteprima si aggiorna e la domanda vi viene riproposta. L'app annuncia il successo solo dopo aver riletto il risultato. **Cos'è una funzionalità e perché è attiva (#1328).** Toccate una funzionalità su una scheda e viene spiegata prima di cambiare qualsiasi cosa: la sua posizione (*processo › sottoprocesso*), cosa offre, cosa **richiede** (con *in ‹processo›* quando il prerequisito è altrove), cosa la **usa**, e perché è attiva o no in questo momento: *attivata*, *attivata e necessaria per …*, oppure *bloccata: richiede …, che è disattivata*. La chiave tecnica si trova sotto **Dettagli tecnici**. **Modificala tra gli interruttori** porta al suo interruttore.
...
<details><summary>La schermata Disponibilità completa e l'elenco delle Funzionalità, ciascuno in un'immagine</summary>
...
vostro indirizzo e-mail. *Vedi le modifiche* mostra, gruppo per gruppo, cosa farebbe qui il modello: i gruppi **Nuovo** sono spuntati, quelli che **cambiano ciò che avete** sono proposti ma non spuntati, quelli **già uguali** non hanno nulla da applicare, e un gruppo che **richiede attenzione** dice perché e non si può scegliere. **Applica N modifiche** applica esattamente ciò che è spuntato — mai il modello intero — e prezzi o ruoli chiedono una conferma in più. Applicando una versione più recente di un modello già applicato, ciò che avete cambiato nel frattempo appare come **Personalizzato qui** e non spuntato: un aggiornamento non annulla mai in silenzio una vostra scelta. Sotto un gruppo che modifica funzionalità, l'anteprima elenca ogni interruttore per **processo aziendale** — *Fatturazione e pagamenti: Fatturazione attiva* — gli stessi processi della schermata Funzionalità; il modello continua a trasportare chiavi di funzionalità e l'app scrive esattamente l'insieme di modifiche mostrato in anteprima (#1330). Livelli, stanze, scrivanie e posti si uniscono **per nome**; non viene mai rimosso ciò che avete già.
...
<details><summary>L'intera schermata dello spazio in un'immagine</summary>
...
**Il colore dello spazio (#1289).** Con *Colori dello spazio* attivo
(*Impostazioni → Funzionalità*), lo spazio porta un colore del marchio
da cui l'app deriva i temi chiaro e scuro — un colore in ingresso, due
temi coerenti, invece di venti valori di cui uno sbagliato. Il colore
arriva con il file dell'assistente di configurazione, un XML di spazio
importato (`brand-color` su `<settings>`) o un modello, e viaggia verso
un gemello come gruppo *Colori*.

L'app mantiene le proprie garanzie di contrasto qualunque colore
riceva: dove una tinta risulterebbe troppo chiara per essere letta su
una superficie, l'app la scurisce — la tinta è vostra, la leggibilità è
del prodotto — e un colore che non potrebbe rendere leggibile viene
rifiutato, indicando la coppia. Il marchio DesKilo, i colori degli
stati dei posti e il banner di produzione non sono mai di uno spazio.
...
<details><summary>L'intera schermata Impostazioni in un'immagine</summary>
