...
- **Modello PDF della fattura** (proprietario, icona matita nell'intestazione Fatture) — scrivi un'introduzione (sopra il blocco del destinatario) e un piè di pagina (condizioni di pagamento, menzioni legali) resi su ogni PDF, con segnaposto come `{{member}}`, `{{number}}`, `{{period}}` o `{{total}}`. L'XML della fattura elettronica non viene mai toccato.
