...
Un account può appartenere a più spazi; passa dall'uno all'altro in **Impostazioni → Profili**, e **contrassegna con la stella quello predefinito** — è il profilo con cui l'app si apre, su ogni dispositivo e anche dopo una reinstallazione (la scelta è salvata con il tuo account). Tutto nell'app è riferito allo spazio attivo.
...
- **Invia notifica** (nella scheda di dettaglio e nelle schede di *Membri e piani*): una breve nota a un altro membro — consegnata come push e come notifica con il tuo nome e il tuo messaggio. Il testo completo resta leggibile in **Eventi → Messaggi**, per il destinatario e per il mittente (il push in sé non trasporta contenuto, per scelta di privacy). Gli admin hanno un megafono **Notifica tutti gli admin** nell'intestazione che raggiunge tutti gli admin, proprietario incluso. Attivabile/disattivabile con la funzionalità *Notifiche tra membri*.
...
- **Disponibilità**: giorni di apertura, giorni di chiusura, granularità — orari liberi di inizio/fine, una griglia di minuti (5/15/30/60), mezze giornate, solo giornate intere oppure orari reali — e l'**orario di lavoro** (inizio, limite di mezza giornata, fine; con orari reali, anche quante ore valgono come mezza giornata e giornata intera).
...
- **Estratti**: mensili, con stato **saldato / da saldare**. Ogni report dell'app — estratto, fatture, proforma, note di credito, i tuoi documenti self-service — offre le stesse tre azioni: **Anteprima rapida** (vedere il documento renderizzato sullo schermo prima di generare il PDF), **Scarica PDF** (salvare localmente) e **Condividi PDF** (consegnarlo a qualsiasi app — WhatsApp, mail, …).
- **Appena il tuo mese è fatturato, decide la fattura**: il tuo estratto mostra una **scheda fattura** — numero, stato, totale, già pagato, residuo — e il mese risulta **saldato** non appena la fattura è pagata, il suo saldo annullato o la sua nota di credito rimborsata, anche se il pagamento che la salda è stato registrato un mese dopo. Una fattura **parzialmente pagata** lascia il mese da saldare esattamente per l'**importo residuo** (è anche quanto addebita *Paga online*). Un mese con **nota di credito** mostra ciò che lo spazio ti deve — nulla da pagare da parte tua.
- **Il tuo conto** — appena possiedi credito disponibile (una nota di credito o pagamenti in eccesso di un mese passato), la scheda Denaro mostra la tua posizione REALE tra i mesi, sopra l'estratto: **credito disponibile**, ogni **fattura aperta** con il residuo, i rimborsi che lo spazio ti deve e la **posizione netta**. Il tuo credito può saldare le fatture aperte: lo spazio lo imputa durante la riconciliazione. I mesi precedenti alla tua adesione non devono nulla e non risultano mai da saldare.
...
- **Aperte** — fatture emesse in attesa di saldo, dalle più vecchie; ciò che attende da oltre 30 giorni diventa rosso, sulla scheda e nella striscia di riepilogo. **Tocca una scheda per leggere la fattura**; i pulsanti agiscono su di essa: **Invia un promemoria** (registra il sollecito e condivide il PDF con un messaggio — la scheda mostra *Sollecitato ×N*), **Segna come errata** (annulla la fattura per correggerla: passa nell'archivio barrata, e una **sostitutiva** ri-deriva lo stesso mese dai dati corretti, citando l'originale) e **Segna come pagata**. **Un pagamento parziale non chiude una fattura**: resta tra le Aperte, con badge *Parzialmente pagata* e l'importo residuo, finché il saldo non viene annullato esplicitamente **tramite il framework di convalida** — un admin/proprietario richiede l'annullamento (con motivo), i validatori confermano e solo allora la fattura passa in archivio come *Parzialmente pagata · saldo annullato*. **Una fattura NEGATIVA è una nota di credito** — i crediti del mese superano i suoi addebiti, quindi lo SPAZIO deve denaro al membro: il suo PDF si intitola *Nota di credito*, non riceve solleciti né riconciliazione con pagamenti del membro; la scheda mostra *Da rimborsare* con **Registra il rimborso** — il versamento si imputa al saldo del membro (convalidato come ogni liquidazione se vale una regola; un rifiuto la riapre) e il documento si chiude come *Rimborsata*. La striscia di riepilogo separa ora le due direzioni del processo di pagamento: *N aperte · X da incassare* conta le fatture positive al loro valore **residuo** (una fattura da 500 € con 280 € pagati conta 220 €), mentre *N da rimborsare · Y* somma le note di credito aperte che lo spazio deve ancora.
- **Editor di report della fattura** (proprietario, icona matita nell'intestazione Fatture) — uno strumento di reporting a bande per il PDF: **banda di intestazione**, **banda del corpo con le righe** e **banda di piè**, ognuna scritta in [Liquid](https://shopify.github.io/liquid/) — `{{ number }}`, `{{ member }}`, condizioni `{% if voided %}…{% endif %}`, cicli `{% for line in lines %}{{ line.label }} | {{ line.amount }}{% endfor %}` — più un markup di riga semplice (`#` titolo, `##` sezione, `>` testo piccolo, `---` divisore, `a | b` riga di tabella, `=` riga in grassetto, `:::` … `|||` … `:::` colonne affiancate — i blocchi indirizzo venditore a sinistra / cliente a destra e il blocco totali allineato a destra di una fattura francese; i modelli forniti seguono esattamente questa struttura). **Ripristina** consegna il layout integrato come esempio funzionante; **Anteprima** rende la fattura più recente come copia filigranata attraverso le bande non salvate. Un **selettore di documento** passa tra la fattura, la **proforma**, l'**estratto del membro** e ogni livello di sollecito — ognuno il proprio report. La proforma usa le bande della fattura finché non la personalizzi; un estratto personalizzato sostituisce il PDF mensile integrato. Una **galleria di modelli** offre quattro report pronti per ogni documento — *Classico*, *Semplice*, *Dettagliato*, *Lettera formale* — scegline uno ed estendilo; ogni modello di fattura porta già le menzioni obbligatorie qui sotto. L'**anteprima rapida** rende il risultato all'istante nell'app (la tua fattura più recente, o dati di esempio simulati se non ce n'è — senza passare da un PDF), e il menu **Anteprima** produce il PDF da **scaricare sul dispositivo** o condividere. L'editor e le regole di sollecito sono raggiungibili anche dalle **Impostazioni dello spazio**.
- **Solleciti (Mahnwesen)** — il proprietario imposta le **regole di sollecito** (icona elenco nell'intestazione Fatture): quanti livelli, giorni fino al primo promemoria, giorni tra i solleciti. Le fatture aperte scadute sono contrassegnate **«Sollecito N da inviare»** e la campanella della scheda diventa rossa — nulla parte mai automaticamente. L'invio genera una **lettera di promemoria di pagamento** (livello 1 amichevole, livelli superiori più fermi) dal modello di quel livello — fornito pronto nella tua lingua e modificabile per livello nell'editor di report con i campi extra `{{ reminder_level }}`, `{{ reminder_date }}` e `{{ days_open }}`. Un modello rotto non blocca mai una fattura (subentra il layout integrato); filigrana di annullo, firma digitale, allegato e numeri di pagina restano fissi — e l'XML della fattura elettronica non viene mai toccato.
- **Fatture valide per legge** — i modelli forniti stampano tutto ciò che una *facture* francese deve riportare: l'identità completa del venditore (nome, **forma giuridica e capitale**, indirizzo, **registro delle imprese**, **n. partita IVA intracomunitaria** — dichiarati in *Impostazioni dello spazio → Identità legale*, nuova sezione **Menzioni di fatturazione**), nome e indirizzo del cliente, numerazione unica e cronologica con data, per riga **quantità, prezzo unitario e aliquota IVA**, il riepilogo IVA per aliquota con **totale netto / IVA / totale**, e le clausole di pagamento: **termini di pagamento**, **sconto per pagamento anticipato**, **penale di mora** e l'**indennità di recupero di 40 €** — ogni clausola lasciata vuota stampa il testo legale predefinito. Le righe opzionali (assicurazione professionale, menzioni particolari come *TVA non applicable, art. 293 B du CGI* tramite il motivo di esenzione) compaiono solo una volta dichiarate. Variabili: `{{ seller_legal_form }}`, `{{ seller_registration }}`, `{{ seller_vat_id }}`, `{{ client_address }}`, `{{ payment_terms }}`, `{{ late_penalty }}`, `{{ recovery_indemnity }}`, `{{ escompte }}`, `{{ insurance }}`, `{{ special_mentions }}`, `{{ net_total }}`, `{{ vat_total }}`, e per riga `{{ line.qty }}`, `{{ line.unit_price }}`, `{{ line.vat_rate }}`, `{{ line.net }}`.
- **Fatturare con o senza IVA — e come associazione** — il **regime IVA** della schermata *Identità legale* è ora applicato end-to-end: solo uno spazio dichiarato *soggetto a IVA* applica un'aliquota ad abbonamenti, supplementi, servizi e pacchetti (lato server, migrazione 0095), e i selettori IVA negli editor di servizi/pacchetti e il riquadro delle aliquote scompaiono sotto ogni altro regime — aliquote residue non possono più tassare nessuno. La nuova scelta **Tipo di organizzazione** (*Impresa* / *Associazione*) copre le regole della *association loi 1901* francese: i documenti di un'associazione omettono le clausole predefinite solo-B2B (penale di mora, indennità di 40 €, sconto — obbligatorie solo tra professionisti; ciò che scrivi viene comunque stampato), i suggerimenti passano a **RNA (W…)** invece del registro delle imprese e *Association loi 1901* come forma giuridica, e il campo di esenzione suggerisce le menzioni adatte — *TVA non applicable, art. 293 B du CGI* o *Exonération de TVA, art. 261, 7-1° du CGI* (servizi ai membri).
- **La suite di report** — tre documenti motore in più accanto alla famiglia fatture, ognuno con i quattro modelli e il proprio template nell'editor: l'**accordo finanziario** (tutti i prezzi validi per un membro — abbonamento, mezza giornata extra, servizi, pacchetti, supplementi di spazi e accessori; proprietari/admin lo inviano dalla scheda del membro, e ogni membro può vedere/scaricare/condividere il proprio da *Finanze → Documenti*), il **report dei pagamenti** (tutto ciò che hai pagato, dichiarato o fatto convalidare nel mese — il tuo piccolo bilancio, self-service sulla stessa riga) e il **report dello spazio** (identità, planimetria, disponibilità, funzionalità e prezzi — *Impostazioni dello spazio → Report dello spazio*).
- **La biblioteca documenti** — *Impostazioni → Documenti*: statuto, guide, bilanci e verbali dello spazio, COLLEGATI dal sistema che già usi — Google Drive, OneDrive, SharePoint, Dropbox, Nextcloud o qualsiasi link https (il drive continua a gestire i propri permessi; l'app non conserva mai credenziali altrui). Ogni voce ha un **ruolo di visibilità**: tutti i membri, admin e proprietari, o solo proprietari — applicato lato server. Admin/proprietari curano col pulsante +; l'interruttore *Biblioteca documenti* attiva il tutto.
- **Archivio** — fatture chiuse, filtrabili per membro e mese e ordinabili; le fatture annullate sono **nascoste per impostazione predefinita** — il chip *Mostra annullate* ripristina la catena di correzione; sotto i filtri è indicato quante fatture corrispondono e **Azzera i filtri** riporta l'archivio intero. Ogni riga porta stato, mese e importo, con **Scarica PDF** lì accanto. **Tocca una riga per aprire la fattura** — posizioni, saldo, destinatario, stato, quale fattura sostituisce o da quale è stata sostituita, il pagamento che l'ha chiusa, i solleciti inviati, la sua firma — e ogni azione ancora permessa, per nome: condividi il PDF, esporta la **fattura elettronica (XML)**, sollecita, segna come pagata, segna come errata, emetti una sostitutiva.
...
**Francia: il FEC.** Uno spazio francese ha una seconda scelta, il **FEC** (*Fichier des Écritures Comptables*) — il file che una verifica là richiede per legge. Non è XML: un file piatto separato da tabulazioni, fatto di **scritture contabili**, denominato `<SIREN>FEC<AAAAMMGG>.txt`, con le 18 colonne obbligatorie nell'ordine obbligatorio. Essendo fatto di scritture non può fare a meno dei numeri di conto: l'esportazione li chiede prima — precompilati con il piano dei conti francese (411 clienti, 706 servizi, 512 banca) e correggibili. Ogni fattura iscrive il credito a fronte del ricavo per l'importo **lordo**; i crediti che ha compensato e il pagamento che l'ha saldata passano in banca con la propria data, collegati al numero di fattura. Le fatture annullate non ci sono: una annullata prima del pagamento non è mai stata contabilizzata, quindi non c'è nulla da stornare. La colonna *nome* segue chi legge — chi emette scorre i membri, un membro scorre i propri numeri di fattura. I membri vedono solo ciò che li riguarda: le fatture emesse, mai una annullata.
...
**Factur-X — un file, due lettori.** Il foglio della fattura elettronica propone per primo **Factur-X (PDF)**: un PDF di fattura dall'aspetto normale con la fattura leggibile dalle macchine *al suo interno* (i dati EN 16931 in formato CII, quello che il formato impone). Una persona lo apre e vede la fattura; una piattaforma lo apre e trova `factur-x.xml`. È ciò che le piccole imprese francesi e tedesche si scambiano davvero, e non richiede un secondo file. L'**XML** da solo resta disponibile sotto, per le piattaforme che lo chiedono nudo.
...
**In DesKilo i prezzi sono IVA inclusa.** Quello che scrivi come prezzo di abbonamento, di servizio o di carnet è quello che il membro paga. Attivare l'IVA non cambia alcun importo dovuto: dice quale parte di quell'importo è imposta. Per questo conto, estratto e quota non si muovono quando aggiungi le aliquote, e per questo nessun totale va riconciliato.

**Configurare le aliquote.** *Impostazioni dello spazio → Identità legale e fatturazione elettronica → **Aliquote IVA***. Un elenco vuoto significa IVA disattivata: è così che ogni spazio comincia. **Usa le aliquote consuete** riempie l'elenco con l'ordinaria e le ridotte del tuo paese come prima bozza: un punto di partenza, non una consulenza fiscale — le aliquote cambiano, e quale prestazione ricada in quale aliquota è una domanda per il tuo commercialista. Un'aliquota è quella **predefinita** (la stella): abbonamenti, superamenti, supplementi e rettifiche la usano, come ogni servizio che non ne ha una propria. Rimuovere un'aliquota non la cancella mai: quella che una fattura o un servizio usa ancora viene conservata, disattivata, così nulla viene tassato di nuovo in silenzio.

**Aliquote per voce.** Un servizio (*Servizi*) e un carnet di giornate (*Fatturazione → Carnet*) portano la propria aliquota, scelta nel loro modulo; lascia **Predefinita dello spazio** e seguirà quella predefinita. Il campo IVA compare solo quando lo spazio ha delle aliquote.

**Cosa cambia su un documento.** Una fattura emessa dopo la creazione delle aliquote porta la ripartizione così come emessa: la tabella delle voci guadagna una colonna di aliquota e sopra il totale il PDF mostra l'**imponibile** e una riga per aliquota. La scheda della fattura nell'app dice lo stesso. La **fattura elettronica (XML)** porta ciò che EN 16931 richiede — un subtotale d'imposta per aliquota, gli importi al netto, la partita IVA del venditore (BR-S-02) — sia in UBL sia in CII: un documento Factur-X è quindi valido anche per un venditore soggetto IVA. **SAF-T** dichiara ogni aliquota nella sua tabella imposte e registra ogni riga al netto con l'imposta accanto; il **FEC** registra il credito al lordo contro il ricavo netto più un conto **IVA incassata** (445710 per impostazione predefinita, e modificabile — nella finestra di esportazione o una volta per sempre nella schermata di identità legale).

**Una fattura già emessa non cambia mai.** Porta le aliquote, l'identità e gli importi con cui è stata firmata: è questo che la rende una fattura. Aggiungere aliquote oggi non mette l'IVA sul documento del mese scorso, e completare l'identità legale oggi non vi aggiunge il numero di registrazione. Se un documento deve portare i nuovi dati, segnalo come **errato** ed emetti una **sostitutiva**: la catena di correzione è visibile su entrambi i documenti, che è esattamente ciò che una verifica vuole vedere.
