...
**The questionnaire speaks five languages (#1366).** No user-facing literal lives in
`web/setup.html`: the page reads every word through `T(key)` from `web/setup_l10n.js`,
generated by `dart run tool/build_setup_l10n.dart`. Feature, permission, validation-domain
and country names come from the app's ARB; the page's own prose comes from
`web/setup_l10n/setup_{en,fr,de,es,it}.json` — a new string goes into all five. Regenerate
and commit after touching either source. `test/lint/setup_l10n_test.dart` gates drift, key
parity and French literals left in the page; the step harness renders every locale and
requires byte-identical XML for the same answers.
