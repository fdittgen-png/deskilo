...
/// A double-tap detector that never enters the gesture arena (#1173).
///
...
/// double-tap window expires. Every descendant tap — the side tabs, and
/// if it wrapped them, the Reserve button itself — is therefore withheld
/// for 300 ms before it fires. Sparkilo shipped that arrangement for one
/// commit and got "the round button sometimes no longer triggers a
/// search" back from the field: a primary action that answers a third of
/// a second late reads as broken, not slow.
///
...
/// with nothing, and delays nothing. The second tap does still reach the
/// child — so this is only safe where the child's own tap is cheap and
/// idempotent (switching to the tab you are already on), and that is why
/// it wraps the tab surface and not the Reserve button.
class ShellBarDoubleTap extends StatefulWidget {
  ...
  /// strip, and somebody drumming two finger-widths apart still means
...
class _ShellBarDoubleTapState extends State<ShellBarDoubleTap> {
  ...
  void _onPointerDown(PointerDownEvent event) {
    ...
    final withinWindow = firstAt != null &&
        event.timeStamp - firstAt <= ShellBarDoubleTap.window;
