...
- **Plantilla del PDF de factura** (propietario, icono de lápiz en la cabecera de Facturas) — escribe una introducción (sobre el bloque del destinatario) y un pie (condiciones de pago, menciones legales) que se renderizan en cada PDF, con marcadores como `{{member}}`, `{{number}}`, `{{period}}` o `{{total}}`. El XML de la factura electrónica nunca se toca.
