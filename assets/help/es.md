...
- **Las invitaciones hablan el idioma del invitado** — la hoja de invitación redacta el mensaje en el idioma elegido (cinco disponibles), por defecto el **idioma del espacio** definido en los *Ajustes del espacio*. El propietario también puede personalizar el texto de la invitación **por idioma**; un idioma vacío usa el mensaje integrado traducido.
...
- **Disponibilidad**: días de apertura, días de cierre, la granularidad — horas de inicio y fin libres, una rejilla de minutos (5/15/30/60), medias jornadas, solo días completos u horas reales — y el **horario laboral** (inicio, límite de media jornada, fin; con horas reales, también cuántas horas cuentan como media jornada y jornada completa).
...
- **Extractos**: mensuales, con estado **saldado / pendiente**. Cada informe de la app — extracto, facturas, proformas, notas de crédito, tus documentos de autoservicio — ofrece las mismas tres acciones: **Vista rápida** (ver el documento renderizado en pantalla antes de generar el PDF), **Descargar PDF** (guardar localmente) y **Compartir PDF** (entregarlo a cualquier app — WhatsApp, correo, …).
- **En cuanto tu mes está facturado, decide la factura**: tu extracto muestra una **tarjeta de factura** — número, estado, total, pagado, pendiente — y el mes pasa a **saldado** en cuanto la factura se paga, se anula su saldo o se reembolsa su nota de crédito, aunque el pago que la salda se registrara un mes después. Una factura **parcialmente pagada** deja el mes pendiente exactamente por el **importe restante** (eso es también lo que cobra *Pagar en línea*). Un mes con **nota de crédito** muestra lo que el espacio te debe — nada que pagar por tu parte.
- **Tu cuenta** — en cuanto tienes crédito disponible (una nota de crédito o pagos sobrantes de un mes pasado), la pestaña Dinero muestra tu posición REAL entre meses, encima del extracto: **crédito a favor**, cada **factura abierta** con su importe restante, los reembolsos que el espacio te debe y la **posición neta**. Tu crédito puede saldar facturas abiertas: el espacio lo imputa al conciliar los pagos. Los meses anteriores a tu adhesión no deben nada y nunca aparecen pendientes.
...
- **Abiertas** — facturas emitidas a la espera de cobro, las más antiguas primero; lo que lleva más de 30 días esperando se pone en rojo, en la tarjeta y en la franja de resumen. **Toca una tarjeta para leer la factura**; los botones actúan sobre ella: **Enviar un recordatorio** (registra el recordatorio y comparte el PDF con un mensaje — la tarjeta muestra *Recordado ×N*), **Marcar como errónea** (anula la factura para corregirla: pasa al archivo tachada, y una **sustituta** vuelve a derivar el mismo mes desde los datos corregidos, referenciando la original) y **Marcar como pagada**. **Un pago parcial no cierra una factura**: permanece en Abiertas, con la insignia *Parcialmente pagada* y el importe pendiente, hasta que el saldo se anule explícitamente **mediante el marco de validación** — un admin/propietario solicita la anulación (con motivo), los validadores confirman y solo entonces la factura pasa al archivo como *Parcialmente pagada · saldo anulado*. **Una factura NEGATIVA es una nota de crédito** — los créditos del mes superan sus cargos, así que el ESPACIO debe dinero al miembro: su PDF se titula *Nota de crédito*, no recibe recordatorios ni conciliación de pagos del miembro; la tarjeta muestra *A reembolsar* con **Registrar el reembolso** — el pago se imputa al saldo del miembro (validado como cualquier liquidación si aplica una regla; un rechazo la reabre) y el documento se cierra como *Reembolsada*. La franja de resumen separa ahora las dos direcciones del proceso de pago: *N abiertas · X pendiente* cuenta las facturas positivas por su valor **restante** (una factura de 500 € con 280 € pagados cuenta 220 €), mientras que *N por reembolsar · Y* suma las notas de crédito abiertas que el espacio aún debe.
- **Editor de informes de factura** (propietario, icono de lápiz en la cabecera de Facturas) — una herramienta de informes por bandas para el PDF: **banda de cabecera**, **banda de cuerpo con las líneas** y **banda de pie**, cada una escrita en [Liquid](https://shopify.github.io/liquid/) — `{{ number }}`, `{{ member }}`, condiciones `{% if voided %}…{% endif %}`, bucles `{% for line in lines %}{{ line.label }} | {{ line.amount }}{% endfor %}` — más un marcado de líneas sencillo (`#` título, `##` sección, `>` letra pequeña, `---` separador, `a | b` fila de tabla, `=` fila en negrita, `:::` … `|||` … `:::` columnas lado a lado — los bloques de dirección vendedor a la izquierda / cliente a la derecha y el bloque de totales alineado a la derecha de una factura francesa; las plantillas incluidas siguen exactamente esa estructura). **Restablecer** entrega el diseño integrado como ejemplo funcional; **Vista previa** renderiza tu factura más reciente como copia con marca de agua a través de las bandas sin guardar. Un **selector de documento** cambia entre la factura, el **proforma**, el **extracto del miembro** y cada nivel de recordatorio — cada uno su propio informe. El proforma usa las bandas de la factura hasta que lo personalices; un extracto personalizado sustituye el PDF mensual integrado. Una **galería de plantillas** ofrece cuatro informes listos para cada documento — *Clásico*, *Sencillo*, *Detallado*, *Carta formal* — elige uno y extiéndelo; cada plantilla de factura ya lleva las menciones obligatorias de abajo. La **vista rápida** renderiza el resultado al instante en la app (tu factura más reciente, o datos de ejemplo simulados si no hay ninguna — sin pasar por un PDF), y el menú **Vista previa** produce el PDF para **descargar en tu dispositivo** o compartir. El editor y las reglas de recordatorio también están accesibles desde los **Ajustes del espacio**.
- **Recordatorios (Mahnwesen)** — el propietario define las **reglas de recordatorio** (icono de lista en la cabecera de Facturas): cuántos niveles, días hasta el primer recordatorio, días entre recordatorios. Las facturas abiertas vencidas se marcan **«Recordatorio N pendiente»** y la campana de la tarjeta se pone roja — nada se envía nunca automáticamente. El envío genera una **carta de recordatorio de pago** (nivel 1 amistoso, niveles superiores más firmes) a partir de la plantilla de ese nivel — incluida lista para usar en tu idioma y editable por nivel en el editor de informes con los campos extra `{{ reminder_level }}`, `{{ reminder_date }}` y `{{ days_open }}`. Una plantilla rota nunca bloquea una factura (retoma el diseño integrado); la marca de anulación, la firma digital, el anexo y los números de página quedan fijos — y el XML de la factura electrónica nunca se toca.
- **Facturas válidas legalmente** — las plantillas incluidas imprimen todo lo que una *facture* francesa debe llevar: la identidad completa del vendedor (nombre, **forma jurídica y capital**, dirección, **registro mercantil**, **n.º de IVA intracomunitario** — declarados en *Ajustes del espacio → Identidad legal*, nueva sección **Menciones de facturación**), nombre y dirección del cliente, numeración única y cronológica con fecha, por línea la **cantidad, el precio unitario y el tipo de IVA**, el resumen de IVA por tipo con **total neto / IVA / total**, y las cláusulas de pago: **condiciones de pago**, **descuento por pronto pago**, **penalización por demora** y la **indemnización de cobro de 40 €** — cada cláusula vacía imprime el texto legal por defecto. Las líneas opcionales (seguro profesional, menciones particulares como *TVA non applicable, art. 293 B du CGI* mediante el motivo de exención) solo aparecen una vez declaradas. Variables: `{{ seller_legal_form }}`, `{{ seller_registration }}`, `{{ seller_vat_id }}`, `{{ client_address }}`, `{{ payment_terms }}`, `{{ late_penalty }}`, `{{ recovery_indemnity }}`, `{{ escompte }}`, `{{ insurance }}`, `{{ special_mentions }}`, `{{ net_total }}`, `{{ vat_total }}`, y por línea `{{ line.qty }}`, `{{ line.unit_price }}`, `{{ line.vat_rate }}`, `{{ line.net }}`.
- **Facturar con o sin IVA — y como asociación** — el **régimen de IVA** de la pantalla *Identidad legal* ahora se aplica de extremo a extremo: solo un espacio declarado *sujeto a IVA* aplica un tipo a suscripciones, suplementos, servicios y paquetes (en el servidor, migración 0095), y los selectores de IVA de los editores de servicios/paquetes y la ficha de tipos desaparecen bajo cualquier otro régimen — los tipos residuales ya no pueden gravar a nadie. La nueva opción **Tipo de organización** (*Empresa* / *Asociación*) cubre las reglas de la *association loi 1901* francesa: los documentos de una asociación omiten las cláusulas por defecto solo-B2B (penalización por demora, indemnización de 40 €, descuento — obligatorias solo entre profesionales; lo que escribas se sigue imprimiendo), las ayudas cambian a **RNA (W…)** en lugar del registro mercantil y *Association loi 1901* como forma jurídica, y el campo de exención sugiere las menciones adecuadas — *TVA non applicable, art. 293 B du CGI* o *Exonération de TVA, art. 261, 7-1° du CGI* (servicios a los miembros).
- **La suite de informes** — tres documentos de motor más junto a la familia de facturas, cada uno con los cuatro modelos y su propia plantilla en el editor: el **acuerdo financiero** (todos los precios vigentes de un miembro — suscripción, media jornada extra, servicios, paquetes, suplementos de espacios y accesorios; propietarios/admins lo envían desde la ficha del miembro, y cada miembro puede ver/descargar/compartir el suyo en *Finanzas → Documentos*), el **informe de pagos** (todo lo pagado, declarado o validado en un mes — tu pequeño balance, autoservicio en la misma fila) y el **informe del espacio** (identidad, plano, disponibilidad, funciones y precios — *Ajustes del espacio → Informe del espacio*).
- **La biblioteca de documentos** — *Ajustes → Documentos*: estatutos, guías, estados financieros y actas del espacio, ENLAZADOS desde el sistema que ya usas — Google Drive, OneDrive, SharePoint, Dropbox, Nextcloud o cualquier enlace https (el drive sigue gestionando sus permisos; la app nunca guarda credenciales ajenas). Cada entrada tiene un **rol de visibilidad**: todos los miembros, admins y propietarios, o solo propietarios — aplicado en el servidor. Admins/propietarios gestionan con el botón +; el interruptor *Biblioteca de documentos* activa todo.
- **Archivo** — facturas cerradas, filtrables por miembro y mes y ordenables; las facturas anuladas están **ocultas por defecto** — el chip *Mostrar canceladas* recupera la cadena de corrección; bajo los filtros se indica cuántas facturas coinciden y **Borrar filtros** devuelve el archivo completo. Cada fila lleva su estado, su mes y su importe, con **Descargar PDF** ahí mismo. **Toca una fila para abrir la factura** — posiciones, saldo, a quién se facturó, en qué estado está, a qué factura sustituye o por cuál fue sustituida, el pago que la cerró, los recordatorios enviados, su firma — y cada acción que aún permite, con su nombre: compartir el PDF, exportar la **factura electrónica (XML)**, recordar, marcar como pagada, marcar como errónea, emitir una sustituta.
