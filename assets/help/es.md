...
**Un escaneo abre la hoja del quiosco.** Leer un código de espacio — la tarjeta QR impresa, o la etiqueta NFC pegada en la silla — ofrece exactamente lo que ofrece el quiosco al tocar ese espacio: las mismas acciones y el mismo periodo derivado de los ajustes del espacio. La única diferencia: ya has iniciado sesión, así que no hay paso de credencial (§4b).
...
| Antes de abrir / fuera de horario (inicio 6:00, 17–21) | ❌ | ❌ | ✅ por defecto — las rejillas son libres; la política **Reservas por minutos dentro del horario laboral** (§8) lo convierte en ❌. Y en **todas** las granularidades, la política **Fuera del horario de apertura** (§8) decide cuánto vale una ventana fuera de horario aceptada — prohibida, gratis o de pago (abajo) |
...
**Fuera del horario de apertura.** Una ventana situada **enteramente** fuera de la jornada laboral — una mañana temprana de 6:00–8:00, una tarde de 17:00–21:00, la llegada espontánea que corre hasta la medianoche local — se rige por una única política del espacio, de tres posiciones (§8). Una ventana que toca el horario laboral, aunque sea un minuto, es una reserva ordinaria y no le concierne.

| Posición | Una reserva (o un registro espontáneo) totalmente fuera de horario |
|---|---|
| **Prohibido** | ❌ rechazada en todas las granularidades — incluida la prórroga vespertina que las granularidades por jornadas permiten siempre |
...
- **Mías / Todos** — tus propias reservas, o las de toda la comunidad. Los puntos bajo un día lo dicen de un vistazo: **rojo** = tienes una reserva, **azul** = otros miembros la tienen, **ambos puntos** = ambas cosas. Hoy va rodeado.
...
- **Políticas de reserva** — cinco entradas que relajan o endurecen las reglas del §4b (la sección sigue la función *Políticas de reserva*); los tres interruptores están todos **desactivados por defecto**:
  ...
  - **Fuera del horario de apertura** — tres posiciones para una ventana situada enteramente fuera de la jornada laboral (una mañana temprana, la tarde, la llegada espontánea hasta la medianoche). *Prohibido* rechaza esas reservas y esos registros espontáneos en todas las granularidades; *Gratis* los permite sin contarlos ni facturarlos nunca; *De pago* (el **defecto**) los cuenta como uso ordinario — salvo un día en el que el miembro ya mantiene una reserva normal dentro del horario: la parte de fuera viaja entonces gratis.
...
- **El cuestionario de configuración** — <https://fdittgen-png.github.io/deskilo/setup.html>: una página independiente (Mac, PC o teléfono; las respuestas se guardan solas en el navegador) que guía a un propietario nuevo por **todos los temas con opciones predefinidas** — identidad (país incl. Noruega, moneda, zona horaria), disponibilidad y granularidad, el plano, todas las funciones (incluidas las declaraciones de IVA), tramos de facturación y niveles de suscripción, bonos, servicios y accesorios, instrucciones de pago, **identidad legal e IVA** (tipo de organización, régimen, tipos habituales del país — el 3,8 % de alojamiento suizo, Noruega, las provincias canadienses, con la nota honesta sobre la sales tax de EE. UU. —, menciones de factura, reglas de recordatorio), la matriz rol → permiso, la regla de validación y los miembros a invitar. **Exportar el XML**: la app importa ajustes, accesorios y plano (*Importar el espacio (XML)*); la sección `<setup>` del archivo lleva el resto. La página también puede **recargar** un archivo exportado para seguir editando.
...
4. En el quiosco, toca un puesto (o **Esta planta**) — se abre **UNA sola hoja** con todo: **Registrarse** ya seleccionado (un toque cambia a **Reservar** o **Salir**), el **periodo ya derivado de los ajustes del espacio**, y el **lector de credenciales activo** abajo. Con medias jornadas, la parte del día en que estás viene preseleccionada (chips Mañana / Tarde / Día para cambiar — una ventana en curso empieza *ahora*, las pasadas están desactivadas, tras el horario queda un único *Resto del día*); con granularidad horaria, selectores De/A ajustados a la rejilla, el inicio de un registro fijado a *ahora*. La hoja **nombra la regla que sigue** — la granularidad y las ventanas horarias de hoy — así que lo que ofrece es exactamente lo que los ajustes permiten; un **día cerrado** se anuncia de entrada con un aviso en vez de fallar al final. Reservar una ventana ya empezada ofrece además **¿Registrarse ya?** (activado por defecto): una sola presentación de la credencial registra la reserva *ya con entrada*. Después presenta la credencial:
...
5. **La credencial ES la confirmación:** ejecuta inmediatamente, y un **recibo que se cierra solo** muestra *a quién* se reconoció, *qué* pasó, *dónde* y *hasta cuándo* — y la pared queda limpia para el siguiente miembro. El camino feliz son dos gestos: toca tu puesto, presenta tu credencial.
