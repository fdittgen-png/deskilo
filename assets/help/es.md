...
- **Hacer no reservable**: en la hoja de reserva, propietarios y admins (con *Los admins pueden bloquear puestos*) retiran el puesto del servicio a partir de ahora — se lee **bloqueado** en el plano hasta liberarlo en la hoja del puesto del editor.
...
En **Ajustes → Administración → Despliegue** — visible mientras la función *Despliegues* está activa, el espacio tiene su gemelo y usted tiene un permiso de despliegue — la configuración y los datos maestros viajan entre los dos lados **entidad por entidad**: *Identidad y datos legales, IVA, Tarifas, Servicios, Paquetes, Accesorios, Sedes, Reglas de reserva, Reglas de validación, Matriz de roles, Reglas de recordatorio, Diseños de documentos, Enlaces de documentos, Días de cierre, Plantillas de invitación, Funciones*. Marque una entidad y lo que necesita se marca con ella (los servicios necesitan los tipos de IVA). **La dirección es el lado en el que está**: desde el lado de desarrollo el botón dice *Desplegar en PROD*, desde el de producción *Desplegar en DEV*. Nada se mueve antes de que una **vista previa** diga, por entidad, qué se añadiría, cambiaría y quitaría al otro lado; una vista previa sin nada que hacer lo dice y no despliega nada. Cada despliegue entra en el **diario** — quién, cuándo, en qué sentido, qué entidades — con *Deshacer* en el último, que repone lo que el otro lado tenía antes. Miembros, reservas, cuentas, facturas, pagos y eventos nunca viajan; tampoco las credenciales ni los contadores de numeración. El plano sigue viajando por el archivo del espacio. Desde #1004 los **planos** también se despliegan — plantas con su sede, precios y reserva completa, oficinas, mesas y puestos con sillas, equipamiento y accesorios, fondos e imágenes del plano — como **fusión**: lo que tiene el otro lado se añade o actualiza, lo que solo tiene este lado se señala y se conserva, porque un puesto puede tener una reserva; las credenciales y los bloqueos nunca viajan, y las imágenes se copian con ellos. Desde #998 el sentido se elige en pantalla: **A** el gemelo o **Desde** él — en el lado de producción, *Desde DEV* es el disparador que trae a producción lo que el lado de desarrollo preparó, y necesita el permiso *Desplegar en producción* en el lado donde está. La lista se agrupa en *Configuración*, *Datos maestros* e *Informes* (los diseños de documentos: cada tipo de informe, ajuste y idioma).
