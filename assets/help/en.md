...
- **Open** — issued invoices awaiting settlement, oldest first; anything waiting longer than 30 days turns red, in the card and in the summary strip. **Tap a card to read the invoice**; the buttons act on it: **Send a reminder** (records the reminder and shares the PDF with a message — the card shows *Reminded ×N*), **Mark as erroneous** (cancels the invoice for correction: it moves to the archive struck through, and a **replacement** re-derives the same month from the corrected data, referencing the original), and **Mark as paid**. **A partial payment does not close an invoice**: it stays on Open, badged *Partially paid* with the remaining amount, until the outstanding remainder is explicitly **cancelled through the validation framework** — an admin/owner requests the write-off (with a reason), the validators confirm, and only then does the invoice move to the archive as *Partially paid · remainder cancelled*. **A NEGATIVE invoice is a credit note (avoir)** — the month's credits exceeded its charges, so the WORKSPACE owes the member: its PDF is titled *Credit note*, it gets no reminders and no member-payment matching; instead the card shows *To refund* with **Record the refund** — the payout books against the member's balance (validated like any settlement when a policy applies; a reject reopens it) and the document closes as *Refunded*. The summary strip now separates the two directions of the payment process: *N open · X outstanding* counts positive invoices at their **remaining** value (a €500 invoice with €280 paid counts €220), while *N to refund · Y* totals the open credit notes the workspace still owes.
- **Invoice report editor** (owner, pencil icon in the Invoices header) — a banded reporting tool for the invoice PDF: a **header band**, a **body band carrying the lines**, and a **footer band**, each written in [Liquid](https://shopify.github.io/liquid/) — `{{ number }}`, `{{ member }}`, `{% if voided %}…{% endif %}` conditions, `{% for line in lines %}{{ line.label }} | {{ line.amount }}{% endfor %}` loops — plus a simple line markup (`#` title, `##` section, `>` small print, `---` divider, `a | b` table row, `=` bold row, `:::` … `|||` … `:::` side-by-side columns — the seller-left / client-right address rows and the right-aligned totals block of a French facture; the shipped templates follow that exact structure). **Reset to default** hands you the built-in layout as a working example; **Preview** renders your newest invoice as a watermarked copy through the unsaved bands. A **document selector** switches between the invoice, the **proforma**, the **member statement** and every reminder level — each its own report. The proforma falls back to the invoice bands until you customize it; a customized statement replaces the built-in monthly-bill PDF. A **Templates gallery** offers four ready-made reports for every document — *Classic*, *Simple*, *Detailed*, *Formal letter* — pick one and extend it; every invoice preset already carries the statutory mentions below. **Quick preview** renders the result instantly in the app (your newest invoice, or simulated sample data when none exists — no PDF round-trip), and the **Preview** menu produces the PDF for **download to your device** or sharing. Both the editor and the reminder rules are also reachable from **Workspace settings**.
- **Reminders (Mahnwesen)** — the owner sets the **reminder rules** (checklist icon in the Invoices header): how many levels, days until the first reminder, days between levels. Overdue open invoices are flagged **"Reminder N due"** and the bell icon on the card turns red — nothing is ever sent automatically. Sending generates a **payment-reminder letter** (level 1 friendly, higher levels firmer) from that level's template — shipped ready-made in your language, and editable per level in the report editor with the extra fields `{{ reminder_level }}`, `{{ reminder_date }}` and `{{ days_open }}`. A broken template never blocks an invoice (the built-in layout takes over); the void watermark, digital signature, annex and page numbers stay fixed — and the e-invoice XML is never touched.
- **Legal-valid invoices** — the shipped templates print everything a French *facture* must carry: your full seller identity (name, **legal form & capital**, address, **trade register**, **VAT number** — declared under *Workspace settings → Legal identity*, new section **Invoice mentions**), the client's name and address, the unique chronological number and date, per line the **quantity, unit price and VAT rate**, the per-rate VAT recap with **total net / VAT / due**, and the payment clauses: **payment terms**, **early-payment discount (escompte)**, **late-payment penalty** and the **€40 recovery indemnity** — each prints the statutory default wording when you leave it empty. Optional lines (professional insurance, special mentions such as *TVA non applicable, art. 293 B du CGI* via the exemption reason) appear only once declared. Template variables: `{{ seller_legal_form }}`, `{{ seller_registration }}`, `{{ seller_vat_id }}`, `{{ client_address }}`, `{{ payment_terms }}`, `{{ late_penalty }}`, `{{ recovery_indemnity }}`, `{{ escompte }}`, `{{ insurance }}`, `{{ special_mentions }}`, `{{ net_total }}`, `{{ vat_total }}`, and per line `{{ line.qty }}`, `{{ line.unit_price }}`, `{{ line.vat_rate }}`, `{{ line.net }}`.
- **Invoicing with or without VAT — and as an association** — the *Legal identity* screen's **VAT regime** is now enforced end to end: only a workspace declared *VAT-registered* ever stamps a rate onto a subscription, supplement, service or package (server-side, migration 0095), and the VAT pickers on the service/package editors plus the VAT-rates tile simply disappear under any other regime — leftover rates can no longer tax anyone. A new **Organization type** choice (*Company / business* vs *Association (non-profit)*) covers the French *association loi 1901* rules: an association's documents drop the B2B-only clause defaults (late penalty, €40 recovery indemnity, escompte are mandatory only between professionals — whatever you type still prints), the hints switch to **RNA (W…)** instead of a trade register and *Association loi 1901* as the legal-form line, and the exemption field suggests the proper mentions — *TVA non applicable, art. 293 B du CGI* (franchise) or *Exonération de TVA, art. 261, 7-1° du CGI* (services to members).
- **The report suite** — three more engine documents beside the invoice family, each with the four presets and its own template in the report editor: the **financial agreement** (every standing price that applies to a member — subscription, extra half-day, services, packages, whole-space and accessory supplements; owners/admins send it from a member's action sheet, and every member can view/download/share their own from *Finances → Documents*), the **payments report** (everything you paid, declared or had validated in a month — your little balance sheet, self-service on the same row), and the **workspace report** (identity, floor-plan counts, availability, features and prices — *Workspace settings → Workspace report*).
- **The document library** — *Settings → Documents*: the workspace's statutes, user guides, financial statements and meeting minutes, LINKED from whatever system you already use — Google Drive, OneDrive, SharePoint, Dropbox, Nextcloud or any https link (the drive keeps managing its own access; the app never stores foreign credentials). Every entry has a **visibility role**: every member, admins and owners, or owners only — enforced server-side, so a member never even downloads a list containing board documents. Admins and owners curate with the + button; a *Document library* feature toggle gates the whole thing.
- **Archive** — closed invoices, filterable by member and month and sortable; cancelled invoices are **hidden by default** — the *Show cancelled* chip brings the correction trail back; the bar under the filters says how many invoices match and **Clear filters** brings the whole archive back. Each row carries its status, its month and its amount, with **Download PDF** right there. **Tap a row to open the invoice** — positions, balance, who it was billed to, where it stands, which invoice it replaces or was replaced by, the payment that closed it, the reminders sent, its signature — and every action it still allows, spelled out: share the PDF, export the **e-invoice (XML)**, remind, mark as paid, mark erroneous, issue a replacement.

**Marking as paid means matching a real payment — or applying a credit.** The dialog also lists the member's **account credits** (credit-note excess): matching one imputes the avoir on the invoice, past months included — the standard alternative to a cash refund, for associations and companies alike. Each credit spends exactly once: one already deducted inside an issued invoice can never settle a second document. The dialog lists the member's registered payments — recorded transfers and confirmed online payments — and you map the invoice to one of them; there is no amount to type. Paid **more**? Create a **credit note** for the excess (a credit on the member's ledger) or force-accept with a mandatory note. Paid **less**? Accept it with a mandatory note. Everyone with invoicing access is notified of paid invoices, and the owner can put an **Invoice payment** validation rule (§6) on them: the match then waits for the quorum — a reject reopens the invoice.
