...
- **Invoice PDF template** (owner, pencil icon in the Invoices header) — write an intro (above the billed-to block) and a footer (payment terms, legal mentions) rendered on every invoice PDF, with placeholders like `{{member}}`, `{{number}}`, `{{period}}` or `{{total}}`. The e-invoice XML is never touched.
