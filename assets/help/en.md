...
- **Invoice report editor** (owner, pencil icon in the Invoices header) — a banded reporting tool for the invoice PDF: a **header band**, a **body band carrying the lines**, and a **footer band**, each written in [Liquid](https://shopify.github.io/liquid/) — `{{ number }}`, `{{ member }}`, `{% if voided %}…{% endif %}` conditions, `{% for line in lines %}{{ line.label }} | {{ line.amount }}{% endfor %}` loops — plus a simple line markup (`#` title, `##` section, `>` small print, `---` divider, `a | b` table row, `=` bold row). **Reset to default** hands you the built-in layout as a working example; **Preview** renders your newest invoice as a watermarked copy through the unsaved bands. A **document selector** switches between the invoice, the **proforma**, the **member statement** and every reminder level — each its own report. The proforma falls back to the invoice bands until you customize it; a customized statement replaces the built-in monthly-bill PDF. A **Templates gallery** offers ready-made reports to start from — *Classic*, *Compact*, *Formal letter*, *Minimal* for the invoice; *Standard*, *Formal letter*, *Short notice* per reminder level — pick one and extend it. **Quick preview** renders the result instantly in the app (your newest invoice, or simulated sample data when none exists — no PDF round-trip), and the **Preview** menu produces the PDF for **download to your device** or sharing. Both the editor and the reminder rules are also reachable from **Workspace settings**.
