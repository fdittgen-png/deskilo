...
- Touchez un membre pour sa **feuille de détail** — rôle, présence, ses **réservations à venir**, et **Envoyer une notification**.
- **Envoyer une notification** : une courte note in-app (jusqu'à 500 caractères) à un autre membre — livrée en push et en notification avec votre nom et votre message. Le texte complet reste lisible sous **Événements → Messages**, pour le destinataire et l'expéditeur (le push lui-même ne porte aucun contenu, par choix de confidentialité). Les admins ont un **mégaphone Notifier tous les admins** dans l'en-tête, qui atteint chaque admin, propriétaire inclus. Débrayable via la fonctionnalité *Notifications entre membres*. Pendant la rédaction, deux puces permettent de **lier une de vos réservations** ou **un espace** (siège, table, bureau ou niveau) — la référence apparaît comme un lien touchable des deux côtés : un lien de réservation ouvre cette réservation, un lien d'espace ouvre la feuille de réservation de l'espace, idéal pour discuter d'une réservation future.
...
**Messages :** la cloche collecte aussi vos notifications de membres (§6) — reçues et envoyées, les plus récentes d'abord. La liste n'affiche que les **64 premiers caractères** ; **touchez un message** pour le lire en entier — émojis compris — avec ses liens de référence actifs (un lien de réservation ouvre cette réservation, un lien d'espace ouvre la feuille de réservation) et les boutons **Répondre**/**Supprimer**. **Balayez à droite** pour répondre à l'expéditeur, **à gauche** pour supprimer — la suppression demande toujours **confirmation** (une diffusion tous-admins reçue ne se supprime pas — elle disparaîtrait pour chaque admin). Les messages non lus comptent sur la cloche et sur l'icône de l'app jusqu'à ouverture de cet écran.
...
4. À la borne : touchez une place (ou **Ce niveau**) → **Pointer**, **Réserver** ou **Départ**. Pour pointer et réserver, une **étape de période** vient d'abord — vous dites *quand* avant de sortir le badge, **aujourd'hui uniquement** et fidèle à la granularité de l'espace : en demi-journées, des puces **Matin / Après-midi / Journée** (une fenêtre en cours démarre *maintenant*, les passées sont désactivées, après les horaires il reste un seul *Reste de la journée*) ; en granularité horaire, des sélecteurs **De/À** calés sur la grille — pointer signifie être là, le début est donc épinglé à *maintenant* et seule la fin bouge. Réserver une fenêtre déjà commencée demande aussi **Pointer tout de suite ?** (activé par défaut — vous êtes, après tout, devant la borne) : confirmer crée la réservation *déjà pointée*, en une seule présentation du badge. Puis présentez le badge :
...
5. **Rien ne se passe sans votre accord :** la borne identifie le badge, ferme les lecteurs et montre un résumé — *qui* elle a reconnu, *ce qui* va se passer, *où* et *quand*. Seul **Confirmer** exécute et rafraîchit le plan ; **Rejeter** abandonne.
