...
- **Envoyer une notification** (dans la fiche détaillée, et sur les fiches membres de *Membres & abonnements*) : une courte note à un autre membre — livrée en push et en notification avec votre nom et votre message. Le texte complet reste lisible sous **Événements → Messages**, pour le destinataire comme pour l'expéditeur (le push lui-même ne transporte aucun contenu, par conception de confidentialité). Les admins disposent d'un mégaphone **Notifier tous les admins** dans l'en-tête, qui atteint tous les admins, propriétaire inclus. Activable/désactivable via la fonctionnalité *Notifications entre membres*.
...
- **Disponibilité** : jours d'ouverture, jours de fermeture, granularité — plage horaire libre, grille de minutes (5/15/30/60), demi-journées, journées entières, ou heures réelles — et les **horaires de travail** (début, limite de demi-journée, fin ; en heures réelles, aussi combien d'heures valent une demi-journée et une journée).
...
- **Relevés** : mensuels, avec statut **réglé / à régler**. Chaque rapport de l'application — relevé, factures, proformas, avoirs, vos documents en libre-service — offre les trois mêmes actions : **Aperçu rapide** (voir le document rendu à l'écran avant tout PDF), **Télécharger le PDF** (enregistrer localement) et **Partager le PDF** (le confier à n'importe quelle appli — WhatsApp, mail, …).
- **Dès que votre mois est facturé, c'est la facture qui décide** : votre relevé affiche une **carte facture** — numéro, état, total, déjà réglé, restant dû — et le mois passe **réglé** dès que la facture est payée, son solde annulé, ou son avoir remboursé, même si le paiement qui la solde a été enregistré un mois plus tard. Une facture **partiellement payée** laisse le mois à régler pour exactement le **restant dû** (c'est aussi ce montant que *Payer en ligne* prélève). Un mois en **avoir** montre ce que l'espace vous doit — rien à payer de votre côté.
- **Votre compte** — dès que vous détenez un crédit disponible (un avoir, ou des paiements excédentaires d'un mois passé), l'onglet Argent affiche votre position RÉELLE toute périodes confondues, au-dessus du relevé : **avoir disponible**, chaque **facture ouverte** avec son restant dû, les remboursements que l'espace vous doit, et la **position nette**. Votre avoir peut solder les factures ouvertes — l'espace l'impute lors du rapprochement des paiements (imputation d'avoir, valable pour les associations comme pour les sociétés). Les mois antérieurs à votre adhésion ne doivent rien et n'affichent jamais « à régler ».
...
- **En cours** — les factures émises en attente de règlement, les plus anciennes d'abord ; au-delà de 30 jours d'attente, l'ancienneté passe au rouge, sur la carte comme dans le bandeau de synthèse. **Touchez une carte pour lire la facture** ; les boutons agissent dessus : **Envoyer un rappel** (enregistre le rappel et partage le PDF avec un message — la carte affiche *Rappelé ×N*), **Marquer comme erronée** (annule la facture pour correction : elle passe aux archives, barrée, et une **facture de remplacement** re-dérive le même mois depuis les données corrigées, en référençant l'originale), et **Marquer comme payée**. **Un paiement partiel ne clôt pas une facture** : elle reste dans En cours, badge *Partiellement payée* avec le restant dû, jusqu'à l'annulation explicite du solde **via le cadre de validation** — un admin/propriétaire demande l'annulation (avec motif), les validateurs confirment, et alors seulement la facture passe aux archives comme *Partiellement payée · solde annulé*. **Une facture NÉGATIVE est un avoir** — les crédits du mois dépassent ses charges, l'ESPACE doit donc de l'argent au membre : son PDF s'intitule *Avoir*, elle ne reçoit ni relances ni rapprochement de paiement membre ; la carte affiche *À rembourser* avec **Enregistrer le remboursement** — le versement s'impute au solde du membre (validé comme tout règlement si une règle s'applique ; un rejet la rouvre) et le document se clôt comme *Remboursée*. Le bandeau de synthèse sépare désormais les deux sens du processus de paiement : *N en cours · X dus* compte les factures positives à leur valeur **restante** (une facture de 500 € payée à 280 € compte 220 €), tandis que *N à rembourser · Y* totalise les avoirs ouverts que l'espace doit encore.
- **Éditeur de rapport de facture** (propriétaire, icône crayon dans l'en-tête Factures) — un outil de reporting à bandes pour le PDF de facture : une **bande d'en-tête**, une **bande de corps portant les lignes** et une **bande de pied**, chacune écrite en [Liquid](https://shopify.github.io/liquid/) — `{{ number }}`, `{{ member }}`, conditions `{% if voided %}…{% endif %}`, boucles `{% for line in lines %}{{ line.label }} | {{ line.amount }}{% endfor %}` — plus un balisage de ligne simple (`#` titre, `##` section, `>` petit texte, `---` séparateur, `a | b` ligne de tableau, `=` ligne en gras, `:::` … `|||` … `:::` colonnes côte à côte — les blocs d'adresse vendeur à gauche / client à droite et le bloc de totaux aligné à droite d'une facture française ; les modèles fournis suivent exactement cette structure). **Réinitialiser** fournit la mise en page intégrée comme exemple fonctionnel ; **Aperçu** rend votre facture la plus récente en copie filigranée à travers les bandes non enregistrées. Un **sélecteur de document** bascule entre la facture, le **proforma**, le **relevé du membre** et chaque niveau de relance — chacun son propre rapport. Le proforma reprend les bandes de la facture tant que vous ne le personnalisez pas ; un relevé personnalisé remplace le PDF de relevé mensuel intégré. Une **galerie de modèles** propose quatre rapports prêts à l'emploi pour chaque document — *Classique*, *Simple*, *Détaillé*, *Lettre formelle* — choisissez-en un et étendez-le ; chaque modèle de facture porte déjà les mentions obligatoires ci-dessous. L'**aperçu rapide** rend le résultat instantanément dans l'app (votre facture la plus récente, ou des données d'exemple simulées s'il n'y en a pas — sans passer par un PDF), et le menu **Aperçu** produit le PDF à **télécharger sur votre appareil** ou à partager. L'éditeur et les règles de relance sont aussi accessibles depuis les **Réglages de l'espace**.
- **Relances (Mahnwesen)** — le propriétaire définit les **règles de relance** (icône liste dans l'en-tête Factures) : nombre de niveaux, jours avant la première relance, jours entre les relances. Les factures ouvertes en retard sont marquées **« Relance N à envoyer »** et la cloche de la carte passe en rouge — rien ne part jamais automatiquement. L'envoi génère une **lettre de rappel de paiement** (niveau 1 amical, niveaux suivants plus fermes) à partir du modèle de ce niveau — fourni prêt à l'emploi dans votre langue, et modifiable par niveau dans l'éditeur de rapport avec les champs supplémentaires `{{ reminder_level }}`, `{{ reminder_date }}` et `{{ days_open }}`. Un modèle cassé ne bloque jamais une facture (la mise en page intégrée reprend) ; le filigrane d'annulation, la signature numérique, l'annexe et les numéros de page restent fixes — et le XML de facture électronique n'est jamais modifié.
- **Factures conformes à la loi** — les modèles fournis impriment tout ce qu'une *facture* française doit porter : l'identité complète du vendeur (nom, **forme juridique et capital**, adresse, **registre du commerce (RCS)**, **n° TVA intracommunautaire** — déclarés dans *Réglages de l'espace → Identité légale*, nouvelle section **Mentions de facturation**), le nom et l'adresse du client, la numérotation unique et chronologique avec la date, par ligne la **quantité, le prix unitaire et le taux de TVA**, le récapitulatif TVA par taux avec **total HT / TVA / TTC**, et les modalités de règlement : **conditions de paiement**, **escompte**, **pénalités de retard** et l'**indemnité forfaitaire de recouvrement de 40 €** — chaque clause laissée vide imprime la mention légale par défaut. Les lignes optionnelles (assurance professionnelle, mentions particulières comme *TVA non applicable, art. 293 B du CGI* via le motif d'exonération) n'apparaissent qu'une fois déclarées. Variables : `{{ seller_legal_form }}`, `{{ seller_registration }}`, `{{ seller_vat_id }}`, `{{ client_address }}`, `{{ payment_terms }}`, `{{ late_penalty }}`, `{{ recovery_indemnity }}`, `{{ escompte }}`, `{{ insurance }}`, `{{ special_mentions }}`, `{{ net_total }}`, `{{ vat_total }}`, et par ligne `{{ line.qty }}`, `{{ line.unit_price }}`, `{{ line.vat_rate }}`, `{{ line.net }}`.
- **Facturer avec ou sans TVA — et en association** — le **régime de TVA** de l'écran *Identité légale* est désormais appliqué de bout en bout : seul un espace déclaré *assujetti à la TVA* applique un taux aux abonnements, suppléments, services et forfaits (côté serveur, migration 0095), et les sélecteurs de TVA des éditeurs de services/forfaits ainsi que la tuile des taux disparaissent sous tout autre régime — des taux résiduels ne peuvent plus taxer personne. Un nouveau choix **Type d'organisation** (*Entreprise* / *Association loi 1901*) couvre les règles associatives : les documents d'une association omettent les clauses par défaut réservées au B2B (pénalités de retard, indemnité de 40 €, escompte — obligatoires uniquement entre professionnels ; ce que vous saisissez s'imprime toujours), les aides basculent sur le **RNA (W…)** au lieu du RCS et *Association loi 1901* comme forme juridique, et le champ d'exonération suggère les mentions adaptées — *TVA non applicable, art. 293 B du CGI* (franchise) ou *Exonération de TVA, art. 261, 7-1° du CGI* (services rendus aux membres).
- **La suite de rapports** — trois documents moteur de plus à côté de la famille facture, chacun avec les quatre modèles et son propre gabarit dans l'éditeur : l'**accord financier** (tous les tarifs qui s'appliquent à un membre — abonnement, demi-journée supplémentaire, services, forfaits, suppléments d'espaces et d'accessoires ; les propriétaires/admins l'envoient depuis la fiche d'un membre, et chaque membre consulte/télécharge/partage le sien depuis *Finances → Documents*), le **rapport des paiements** (tout ce que vous avez payé, déclaré ou fait valider dans le mois — votre petit bilan, en libre-service sur la même ligne), et le **rapport de l'espace** (identité, plan, disponibilité, fonctionnalités et tarifs — *Réglages de l'espace → Rapport de l'espace*).
- **La bibliothèque de documents** — *Réglages → Documents* : statuts, guides, états financiers et comptes rendus de l'espace, LIÉS depuis le système que vous utilisez déjà — Google Drive, OneDrive, SharePoint, Dropbox, Nextcloud ou tout lien https (le drive garde la gestion de ses accès ; l'app ne stocke jamais d'identifiants tiers). Chaque entrée a un **rôle de visibilité** : tous les membres, admins et propriétaires, ou propriétaires seuls — appliqué côté serveur. Les admins/propriétaires gèrent via le bouton + ; la fonctionnalité *Bibliothèque de documents* active l'ensemble.
- **Archives** — les factures clôturées, filtrables par membre et par mois, et triables ; les factures annulées sont **masquées par défaut** — la puce *Afficher les annulées* fait réapparaître la chaîne de correction ; sous les filtres, une ligne indique combien de factures correspondent et **Réinitialiser les filtres** ramène l'ensemble. Chaque ligne porte son statut, son mois et son montant, avec **Télécharger le PDF** directement. **Touchez une ligne pour ouvrir la facture** — positions, solde, destinataire, état, quelle facture elle remplace ou par laquelle elle a été remplacée, le paiement qui l'a clôturée, les rappels envoyés, sa signature — et chacune des actions encore permises, en clair : partager le PDF, exporter la **facture électronique (XML)**, relancer, marquer comme payée, marquer comme erronée, émettre un remplacement.

**Marquer comme payée, c'est rapprocher un paiement réel — ou imputer un avoir.** La boîte de dialogue liste aussi les **avoirs du membre** (excédent de note de crédit) : en rapprocher un impute l'avoir sur la facture, mois passés compris — l'alternative classique au remboursement, pour les associations comme pour les sociétés. Chaque crédit ne se dépense qu'une fois : un crédit déjà déduit dans une facture émise ne peut jamais solder un second document. Le dialogue liste les paiements enregistrés du membre — virements saisis et paiements en ligne confirmés — et vous associez la facture à l'un d'eux ; il n'y a aucun montant à saisir. Payé **plus** ? Créez un **avoir pour l'excédent** (un crédit sur le registre du membre) ou forcez l'acceptation avec une note obligatoire. Payé **moins** ? Acceptez avec une note obligatoire. Toutes les personnes ayant accès à la facturation sont notifiées des factures payées, et le propriétaire peut poser une règle de validation **Paiement de facture** (§6) : le rapprochement attend alors le quorum — un rejet rouvre la facture.
...
**Répéter sans risque.** Un espace peut enregistrer, à côté du point de production, des **points de test** (l'UAT de la plateforme ou une cible dev). Avec le **mode développeur** de l'espace activé (un réglage à l'échelle de l'espace que seuls propriétaires et admins peuvent basculer, sous Réglages → Avancé), l'envoi propose le choix de l'environnement, une soumission de test est marquée comme telle dans l'historique de transmission de la facture, et le point de production n'est jamais utilisé pour une répétition — un environnement de test non configuré refuse, sans jamais se rabattre.
